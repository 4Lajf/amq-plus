...
import { validateAllocation } from '$lib/utils/commonValidators.js';
import { createValidationContext, getMode, buildEntry } from '../FilterBase.js';
import { allocateToTotal, analyzeGroup } from '$lib/utils/mathUtils.js';
import { extractSongsAndTypesDisplay } from '$lib/utils/displayUtils.js';
...
function validateSongsAndTypes(value, context) {
  ...
  for (const key of ['random', 'watched', 'unwatched']) {
    const cfg = sel[key];
    if (!cfg) continue;
...
function resolveSongsAndTypes(node, context, rng) {
  ...
  // Build unwatched selection entry (optional - songs NOT on player's list from masterlist)
  if (sel.unwatched) {
    if (sel.unwatched.random) {
      let min, max;
      if (mode === 'percentage') {
        min = sel.unwatched.percentageMin ?? sel.unwatched.min ?? 0;
        max = sel.unwatched.percentageMax ?? sel.unwatched.max ?? 0;
      } else {
        min = sel.unwatched.countMin ?? sel.unwatched.min ?? 0;
        max = sel.unwatched.countMax ?? sel.unwatched.max ?? 0;
      }
      selEntries.push({
        label: 'unwatched',
        kind: /** @type {'range'} */ ('range'),
      ...
      let val;
      if (mode === 'percentage') {
        val = sel.unwatched.percentage ?? sel.unwatched.value ?? 0;
      } else {
        val = sel.unwatched.count ?? sel.unwatched.value ?? 0;
      }
      selEntries.push({
        label: 'unwatched',
        kind: /** @type {'static'} */ ('static'),
  ...
  const resolvedSelection = {
    ...
    unwatched: (() => {
      const val = selAllocation.get('unwatched');
      return val ?? 0; // Optional, default to 0 if not present
    })()
  };
