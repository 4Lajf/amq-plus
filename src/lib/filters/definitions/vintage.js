...
import { validateVintageRanges, validateAllocation } from '$lib/utils/commonValidators.js';
import { formatVintageRange } from '$lib/utils/commonDisplayUtils.js';
import { allocateToTotal } from '$lib/utils/mathUtils.js';
...
function extractVintage(value, context) {
  ...
  const formattedRanges = ranges.map((r) => {
    ...
    return {
      ...
      to: {
        year: r.to?.year || 2025,
        season: r.to?.season || 'Fall',
        present: r.to?.present || false
      },
...
function resolveVintage(node, context, rng) {
  ...
  // Get current year and season for "Present" resolution and default random range
  const now = new Date();
  ...
  for (const range of value.ranges) {
    ...
    // Resolve "Present" to current date if needed
    const effectiveTo = range.to.present
      ? { season: currentSeason, year: currentYear }
      : range.to;

    if (!effectiveTo.season || !effectiveTo.year) {
      throw new Error('range.to.season and range.to.year are required');
    }

    const label = range.to.present
      ? `${range.from.season} ${range.from.year} - Present`
