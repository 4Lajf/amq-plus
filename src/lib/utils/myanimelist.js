...
export async function fetchMALData(username, options = {}, clientId = null) {
	...
	try {
		...
		for (const status of statusesToFetch) {
			...
			// Delay between different statuses
		if (statusesToFetch.indexOf(status) < statusesToFetch.length - 1) {
			await sleep(500);
		}
	}
...
async function fetchMALPage(username, offset, limit, status, clientId) {
	...
	// Include R+/R18 anime - MAL API excludes them by default
