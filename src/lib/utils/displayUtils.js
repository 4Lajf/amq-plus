 ...
 *
 * @param {import('./defaultNodeSettings.js').BasicSettings | Record<string, any>} value - Basic settings value object
...
export function extractBasicSettingsDisplay(value) {
	...
	const guess = extractValue(v.guessTime);
	const extraGuess = extractValue(v.extraGuessTime);
	const sample = extractValue(v.samplePoint);
	const speed = extractValue(v.playbackSpeed);

	return {
		scoring: v.scoring?.value ?? v.scoring,
		answering: v.answering?.value ?? v.answering,
		players: Number(v.players?.value ?? v.players),
		teamSize: Number(v.teamSize?.value ?? v.teamSize),
		guessTime: guess.useRange
			? { kind: 'range', min: Number(guess.min), max: Number(guess.max) }
			: { kind: 'static', value: Number(guess.staticValue ?? guess.value ?? (typeof guess === 'number' ? guess : 0)) },
		extraGuessTime: extraGuess.useRange
			? { kind: 'range', min: Number(extraGuess.min), max: Number(extraGuess.max) }
			: { kind: 'static', value: Number(extraGuess.staticValue ?? extraGuess.value ?? (typeof extraGuess === 'number' ? extraGuess : 0)) },
		samplePoint: sample.useRange
			? { kind: 'range', min: Number(sample.start), max: Number(sample.end) }
			: { kind: 'static', value: Number(sample.staticValue ?? sample.value ?? (typeof sample === 'number' ? sample : 0)) },
		playbackSpeed:
			speed.mode === 'static'
				? { kind: 'static', value: Number(speed.staticValue ?? (typeof speed === 'number' ? speed : 0)) }
				: {
					kind: 'random',
					values: Array.isArray(speed.randomValues) ? speed.randomValues : []
				},
		modifiers: Array.isArray(v.modifiers?.value)
			...
			: Array.isArray(v.modifiers)
				? v.modifiers
				: [],
		preventSameSongSpam: v.preventSameSongSpam?.value ?? v.preventSameSongSpam ?? false,
		duplicateShows: v.duplicateShows?.value ?? v.duplicateShows ?? true
	};
}
 ...
 *
 * @param {import('./defaultNodeSettings.js').NumberOfSongsSettings | Record<string, any>} value - Number of songs value object
 ...
 * @param {import('./defaultNodeSettings.js').AnimeTypeSettings | Record<string, any>} value - Anime types value object
 * @param {number | {kind: 'range', min?: number, max?: number} | {kind: 'static', value: number} | Record<string, any>} inheritedSongCount - Inherited song count configuration
 * @returns {Object|null} Formatted display values with allocation analysis
 */
 ...
 * @param {import('./defaultNodeSettings.js').SongDifficultySettings | Record<string, any>} value - Song difficulty value object
 * @param {number | {kind: 'range', min?: number, max?: number} | {kind: 'static', value: number} | Record<string, any>} inheritedSongCount - Inherited song count configuration
 ...
 * @param {import('./defaultNodeSettings.js').VintageSettings | Record<string, any>} value - Vintage value object
 * @param {number | {kind: 'range', min?: number, max?: number} | {kind: 'static', value: number} | Record<string, any>} inheritedSongCount - Inherited song count configuration
 * @returns {Object} Formatted display values
 */
 ...
 * @param {import('./defaultNodeSettings.js').ScoreRangeSettings | Record<string, any>} value - Player score value object
 * @param {number | {kind: 'range', min?: number, max?: number} | {kind: 'static', value: number} | Record<string, any>} inheritedSongCount - Inherited song count configuration
 * @returns {Object} Formatted display values
 */
 ...
 * @param {import('./defaultNodeSettings.js').ScoreRangeSettings | Record<string, any>} value - Anime score value object
 * @param {number | {kind: 'range', min?: number, max?: number} | {kind: 'static', value: number} | Record<string, any>} inheritedSongCount - Inherited song count configuration
 * @returns {Object} Formatted display values
 */
 ...
 * @param {import('./defaultNodeSettings.js').SongsAndTypesSettings | Record<string, any>} value - Songs and types value object
 * @param {number | {kind: 'range', min?: number, max?: number} | {kind: 'static', value: number} | Record<string, any>} inheritedSongCount - Inherited song count configuration
