...
export function formatVintageRange(range) {
  ...
  const to = range.to.present ? 'Present' : `${range.to.season} ${range.to.year}`;
 ...
 * @returns {string}
 */
export function formatSongCategoriesSimple(value, rows = ['openings', 'endings', 'inserts'], cols = ['standard', 'instrumental', 'chanting', 'character', 'noCategory']) {
  if (!value) {
