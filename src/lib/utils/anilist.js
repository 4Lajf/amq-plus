...
/** Delay between AniList API batch requests (ms) - helps avoid 429 Too Many Requests */
const ANILIST_BATCH_DELAY_MS = 4000;
...
async function fetchBatchMetadataByAnilistIds(anilistIds, onProgress = null) {
	...
	for (let i = 0; i < anilistIds.length; i += SUB_BATCH_SIZE) {
		...
		try {
			...
			if (!response.ok) {
				// AniList returns 404 if any ID in the batch is not found
				// but it may still return data for the other IDs
				if (response.status === 404 && data.data) {
					console.warn(
						`Sub-batch ${currentSubBatch}/${totalSubBatches} returned 404, but processing partial data.`
					);
				} else {
					console.error(
					...
					// Continue to next batch if it's a fatal error
					continue;
				}
			}
		...
		if (i + SUB_BATCH_SIZE < anilistIds.length) {
			await new Promise((resolve) => setTimeout(resolve, ANILIST_BATCH_DELAY_MS));
		}
	}
