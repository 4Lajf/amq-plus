 ...
 * @property {BasicSettingField} [preventSameSongSpam] - Prevent same song spam (room-wide)
 */
...
/**
 * Song selection configuration (for random/watched/unwatched).
 ...
 * @property {SongSelectionConfig} songSelection.random - Random selection (from anywhere - can be on list or not)
 * @property {SongSelectionConfig} songSelection.watched - Watched selection (from player's list)
 * @property {SongSelectionConfig} songSelection.unwatched - Unwatched selection (from masterlist, NOT on player's list)
 */
 ...
 * @property {VintageConfig & {present?: boolean}} to - End vintage (present=true means "Present")
 * @property {number} percentage - Percentage allocation
 * @property {number} count - Song count
 ...
 * @property {boolean} openings.noCategory - Include songs with no category
 ...
 * @property {boolean} endings.noCategory - Include songs with no category
 ...
 * @property {boolean} inserts.noCategory - Include songs with no category
 * @property {'simple' | 'advanced'} viewMode - View mode
 * @property {'percentage' | 'count'} mode - Allocation mode
 * @property {Object} advanced - Advanced mode settings
 ...
 * @property {SongCategoryItemConfig} advanced.openings.noCategory - No category configuration
 ...
 * @property {SongCategoryItemConfig} advanced.endings.noCategory - No category configuration
 ...
 * @property {SongCategoryItemConfig} advanced.inserts.noCategory - No category configuration
 */
 ...
 * @property {boolean} [savedListHasOnlyListSource] - True when selected saved list contains only songs with source "list"
 ...
 * Filters by song categories (Standard, Instrumental, Chanting, Character, No Category).
 * @type {SongCategoriesSettings}
 */
export const SONG_CATEGORIES_DEFAULT_SETTINGS = {
	openings: { standard: true, instrumental: true, chanting: true, character: true, noCategory: true },
	endings: { standard: true, instrumental: true, chanting: true, character: true, noCategory: true },
	inserts: { standard: true, instrumental: true, chanting: true, character: true, noCategory: true },
	viewMode: 'simple', // 'simple' | 'advanced'
	mode: 'count', // 'percentage' | 'count'
	advanced: {
		openings: {
...
export const SONG_LIST_DEFAULT_SETTINGS = {
	...
	// For provider mode
	providerImport: {
		providerType: 'amq-export', // 'amq-export' | 'joseph-song-ui' | 'blissfullyoshi-ranked' | 'kempanator-answer-stats' | 'csl-import'
		fileData: null,
		processedData: null
	},
...
/**
 * Default settings for Negative Song List node.
 * Uses the same selection modes as Song List but acts as exclusion source.
 * @type {SongListSettings}
 */
export const NEGATIVE_SONG_LIST_DEFAULT_SETTINGS = {
...
/**
 * Anime Title Length settings.
 * @typedef {Object} AnimeTitleLengthSettings
 * @property {'disabled' | 'range'} mode - Filter mode (disabled = no-op)
 * @property {number} minWords - Minimum word count (0-99)
 * @property {number} maxWords - Maximum word count (0-99)
 ...
 * @property {boolean} reverse - If true, keep least popular instead
 */

/**
 * Default settings for Anime Title Length filter node.
 * Filters anime by title word count.
 * @type {AnimeTitleLengthSettings}
 */
export const ANIME_TITLE_LENGTH_DEFAULT_SETTINGS = {
	mode: 'disabled',
	minWords: 0,
	maxWords: 99,
	titleField: 'auto'
...
/**
 * Default settings for Popularity filter node.
 * Filters songs by anime popularity rank.
 * @type {PopularitySettings}
 */
export const POPULARITY_DEFAULT_SETTINGS = {
	mode: 'percentage',
	percentageValue: 100,
	countValue: 500,
	reverse: false
 ...
 * @type {Object.<string, RouterSettings | BasicSettings | SongsAndTypesSettings | VintageSettings | SongDifficultySettings | ScoreRangeSettings | AnimeTypeSettings | SongCategoriesSettings | GenresTagsSettings | SelectionModifierSettings | NumberOfSongsSettings | SongListSettings | BatchUserListSettings | LiveNodeSettings | AnimeTitleLengthSettings | PopularitySettings | SourceSelectorSettings>}
 */
export const DEFAULT_NODE_SETTINGS = {
	...
	'negative-song-list': NEGATIVE_SONG_LIST_DEFAULT_SETTINGS,
	'batch-user-list': BATCH_USER_LIST_DEFAULT_SETTINGS,
	'live-node': LIVE_NODE_DEFAULT_SETTINGS,
	'anime-title-length': ANIME_TITLE_LENGTH_DEFAULT_SETTINGS,
	popularity: POPULARITY_DEFAULT_SETTINGS,
