...
export function validateVintageRanges(ranges) {
  ...
  for (const range of ranges) {
    ...
    // @ts-ignore - present property exists on VintageRangeConfig.to
    const isPresent = range?.to?.present === true;
    const toYear = isPresent ? null : Number(range?.to?.year);
    const fromSeason = range?.from?.season;
    const toSeason = isPresent ? null : range?.to?.season;

    if (!Number.isFinite(fromYear)) {
      result.addError('Invalid start year in vintage range');
      continue;
    }

    if (!isPresent && !Number.isFinite(toYear)) {
      result.addError('Invalid end year in vintage range');
      continue;
    }

    if (fromYear < 1900 || (!isPresent && toYear > 2100)) {
      result.addError('Vintage years must be between 1900 and 2100');
    }

    if (!seasons.has(fromSeason) || (!isPresent && !seasons.has(toSeason))) {
