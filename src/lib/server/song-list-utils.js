 ...
 * 
 * @param {Object} configurationData - The quiz configuration data (v2 routes format)
 * @returns {string[]} Array of unique song list IDs
 */
export function extractSongListIds(configurationData) {
  if (!configurationData?.routes) return [];

  const listIds = [];

  for (const route of configurationData.routes) {
    for (const src of [...(route.sources || []), ...(route.negativeSources || [])]) {
      if (src.mode === 'saved-lists' && src.selectedListId) {
        listIds.push(src.selectedListId);
      }
    }
  }
