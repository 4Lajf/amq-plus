 ...
 * @property {boolean} [present] - Whether to use current date at runtime (for 'to' vintage only)
 */
 ...
 * @property {number|null} [popularity] - Anime popularity rank (number of users who have it on their list)
 ...
 * @property {string|null} [HQ] - High quality video filename (null if missing)
 * @property {string|null} [MQ] - Medium quality video filename (null if missing)
 * @property {string|null} [audio] - Audio filename (null if missing)
 ...
 * @property {string} [_sourceType] - Internal property: source type ('random' | 'watched' | 'unwatched')
 */

/**
 * @typedef {Object} VintageRange
 * @property {Vintage} from - Start vintage
 * @property {Vintage} to - End vintage (may have present property)
 ...
 * @property {string} nodeType - Node type ('song-list', 'batch-user-list', 'live-node', 'negative-song-list')
...
/**
 * Anime title length filter settings (from 'anime-title-length' filter)
 * Documentation only - describes settings structure for this filter type
 * @typedef {Object} AnimeTitleLengthSettings
 * @property {'disabled'|'range'} mode - Filter mode
 * @property {number} minWords - Minimum word count
 * @property {number} maxWords - Maximum word count
 * @property {'auto'|'english'|'romaji'} titleField - Which title field to check
 */

/**
 * Popularity filter settings (from 'popularity' filter)
 * Documentation only - describes settings structure for this filter type
 * @typedef {Object} PopularitySettings
 * @property {'percentage'|'count'} mode - Selection mode
 * @property {number} percentageValue - Top percentage to keep (1-100)
 * @property {number} countValue - Top count to keep (>=1)
 * @property {boolean} reverse - If true, pick least popular instead of most popular
 */
 ...
 * @property {SongListSettings[]} [negativeSongLists] - Array of exclusion song list configurations
 ...
 * @property {{min:number,max:number}|null} [relaxRange] - Optional relaxation bounds from unresolved random ranges
 ...
 * @property {{poolCount: number, maxPopularity: number|null, sortedPopularities: number[]}|null} [popularityReference] - Pre-popularity pool context
 */
 ...
 * @property {number} constraintFeasibleSongCount - Songs matching active basket constraints (before per-basket fill limits)
 * @property {number|null} constraintUniqueAnimeCount - Unique anime count among constraint-feasible songs
 * @property {number} constraintFeasibleCap - Maximum feasible songs after hard constraints (includes duplicate show rule)
 ...
 * @property {Array} songOverlapMap - Mapping of annSongId to users who know it
 */
...
/**
 * Sort songs by annSongId for deterministic selection
 * This ensures the same songs are always selected regardless of input order
 * @template T
 * @param {T[]} array - Array of songs to sort
 * @returns {T[]} Sorted array (returns new array)
 */
function stableSortByAnnSongId(array) {
  return [...array].sort((a, b) => {
    // @ts-ignore - annSongId may be string or number
    const idA = Number(a.annSongId) || 0;
    // @ts-ignore - annSongId may be string or number
    const idB = Number(b.annSongId) || 0;
    return idA - idB;
  });
}
...
export function isInVintageRange(vintage, from, to) {
  ...
  // Handle "present" option which resolves to current date at runtime
  let toYear = to.year;
  let toSeason = to.season;

  if (to.present === true) {
    const now = new Date();
    toYear = now.getFullYear();
    const month = now.getMonth();
    if (month >= 0 && month <= 2) toSeason = 'Winter';
    else if (month >= 3 && month <= 5) toSeason = 'Spring';
    else if (month >= 6 && month <= 8) toSeason = 'Summer';
    else toSeason = 'Fall';
  }

  const toValue = toYear * 4 + seasons.indexOf(toSeason);
...
/**
 * Extract only positive source settings from simulated configuration.
 * @param {SimulatedConfig} simulatedConfig - Simulated quiz configuration
 * @returns {SongListSettings[]} Array of positive source configurations
 */
function extractPositiveSongListSettings(simulatedConfig) {
  return (simulatedConfig.songLists || []).filter(
    (settings) => settings?.nodeType !== 'negative-song-list'
  );
}

/**
 * Extract negative song list settings from simulated configuration.
 * Falls back to negative entries in songLists for backward compatibility.
 * @param {SimulatedConfig} simulatedConfig - Simulated quiz configuration
 * @returns {SongListSettings[]} Array of negative song list configurations
 */
function extractNegativeSongListSettings(simulatedConfig) {
  const dedicatedNegativeNodes = simulatedConfig.negativeSongLists || [];
  if (dedicatedNegativeNodes.length > 0) {
    return dedicatedNegativeNodes;
  }
...
/**
 * Build a stable identity key for song exclusion.
 * Prefer annSongId, then fall back to MAL + title/type tuple.
 * @param {Song} song - Song object
 * @returns {string|null} Identity key or null when no stable identity exists
 */
function getSongIdentityKey(song) {
  if (!song) return null;

  if (song.annSongId !== undefined && song.annSongId !== null && song.annSongId !== '') {
    return `ann:${String(song.annSongId)}`;
  }

  const malId = song.malId ?? song.linked_ids?.myanimelist ?? song.sourceAnime?.malId ?? null;
  const songName = typeof song.songName === 'string' ? song.songName.trim().toLowerCase() : '';
  const songType = typeof song.songType === 'string' ? song.songType.trim().toLowerCase() : '';

  if (malId !== null && songName && songType) {
    return `mal:${String(malId)}|name:${songName}|type:${songType}`;
  }
...
/**
 * Load songs for exclusion-only negative source nodes.
 * @param {SimulatedConfig} simulatedConfig - Simulated configuration
 * @param {typeof fetch} fetchFn - Fetch function
 * @returns {Promise<{songs: Song[], loadingErrors: LoadingError[]}>}
 */
async function loadNegativeSourceSongs(simulatedConfig, fetchFn = fetch) {
  const negativeSongListSettings = extractNegativeSongListSettings(simulatedConfig);
  if (negativeSongListSettings.length === 0) {
    return { songs: [], loadingErrors: [] };
  }

  const negativeConfig = {
    ...simulatedConfig,
    songLists: negativeSongListSettings,
    filters: [] // Exclusion pool should be loaded from source definitions only
  };

  const { songs, loadingErrors } = await loadSourceSongs(negativeConfig, fetchFn);
  return { songs, loadingErrors };
}
...
const FILTER_REGISTRY = {
  'anime-title-length': {
    applyGlobalFilter: (songs, settings) => {
      // If disabled, return all songs
      if (settings.mode === 'disabled') {
        return { songs, stats: null };
      }

      const { minWords, maxWords, titleField } = settings;
      const beforeCount = songs.length;
      ...
      const filtered = songs.filter(song => {
        ...
        const englishTitle = song.sourceAnime?.title?.english || song.animeENName || '';
        const romajiTitle = song.sourceAnime?.title?.romaji || song.animeJPName || '';

        if (titleField === 'english') {
          titleToCheck = englishTitle;
        } else if (titleField === 'romaji') {
          titleToCheck = romajiTitle;
        } else {
          // Auto: prefer English, fallback to Romaji
          titleToCheck = englishTitle || romajiTitle;
        }

        // If no title found, it has 0 words. 
        // We include it only if minWords is 0.
        if (!titleToCheck) {
          return 0 >= minWords && 0 <= maxWords;
        }

        const wordCount = titleToCheck.trim().split(/\s+/).filter(w => w.length > 0).length;

        return wordCount >= minWords && wordCount <= maxWords;
      });

      console.log(`[GLOBAL FILTERS] Anime Title Length filter: ${filtered.length}/${beforeCount} songs (range: ${minWords}-${maxWords}, field: ${titleField})`);

      const stats = recordFilterStat('Anime Title Length', beforeCount, filtered.length, {
        minWords, maxWords, titleField
      });

      return { songs: filtered, stats };
    },

    metadata: { name: 'Anime Title Length', category: 'content' }
  },

  'popularity': {
    applyGlobalFilter: (songs, settings) => {
      const mode = settings?.mode === 'count' ? 'count' : 'percentage';
      const reverse = Boolean(settings?.reverse);
      const beforeCount = songs.length;

      const songsWithPopularity = songs.filter((song) => Number.isFinite(song.sourceAnime?.popularity));
      const totalWithPopularity = songsWithPopularity.length;

      // Nothing to rank -> treat as no-op.
      ...
      let selectedCount = 0;
      if (mode === 'count') {
        const rawCountValue = Number(settings?.countValue ?? 500);
        const countValue = Number.isFinite(rawCountValue) ? rawCountValue : 500;
        selectedCount = Math.max(1, Math.min(totalWithPopularity, Math.floor(countValue)));
      } else {
        const rawPercentageValue = Number(settings?.percentageValue ?? 100);
        const percentageValue = Number.isFinite(rawPercentageValue) ? rawPercentageValue : 100;
        const normalizedPercentage = Math.max(1, Math.min(100, percentageValue));
        selectedCount = Math.round(totalWithPopularity * normalizedPercentage / 100);
        selectedCount = Math.max(1, Math.min(totalWithPopularity, selectedCount));
      }

      // Include missing popularity only when selection covers all ranked songs.
      const isNoOp = selectedCount >= totalWithPopularity;
      if (isNoOp) {
        return { songs, stats: null };
      }

      const sortedByPopularity = [...songsWithPopularity].sort((a, b) => {
        const aPopularity = Number(a.sourceAnime?.popularity ?? 0);
        const bPopularity = Number(b.sourceAnime?.popularity ?? 0);
        return reverse ? aPopularity - bPopularity : bPopularity - aPopularity;
      });

      const selectedSongs = new Set(sortedByPopularity.slice(0, selectedCount));
      const filtered = songs.filter((song) => selectedSongs.has(song));

      console.log(
        `[GLOBAL FILTERS] Popularity filter: ${filtered.length}/${beforeCount} songs (mode: ${mode}, ` +
        `${mode === 'count' ? `count=${selectedCount}` : `percentage=${settings?.percentageValue ?? 100}%`}, ` +
        `${reverse ? 'least-popular first' : 'most-popular first'})`
      );

      const stats = recordFilterStat('Popularity', beforeCount, filtered.length, {
        mode,
        percentageValue: mode === 'percentage' ? Number(settings?.percentageValue ?? 100) : null,
        countValue: mode === 'count' ? Number(settings?.countValue ?? 500) : null,
        selectedCount,
        reverse,
        missingPopularityExcluded: beforeCount - totalWithPopularity
      });

      return { songs: filtered, stats };
    },

    metadata: { name: 'Popularity', category: 'content' }
  },
  ...
  'anime-type': {
    ...
    buildBaskets: (settings, targetSourceId, context) => {
      ...
      Object.entries(types || {}).forEach(([type, quota]) => {
        // Prioritize types (resolved values) over typesRanges (unresolved ranges)
        const rangeOrValue = quota || typesRanges?.[type];
  ...
  'song-categories': {
    applyGlobalFilter: (songs, settings) => {
      ...
      const filtered = songs.filter(song => {
        ...
        let category = (song.songCategory || '').toLowerCase();
        // Normalize "no category" (with space) to "noCategory" (camelCase)
    ...
    buildBaskets: (settings, targetSourceId, context) => {
      ...
      Object.entries(categories || {}).forEach(([songType, cats]) => {
        Object.entries(cats).forEach(([category, quota]) => {
          // Prioritize categories (resolved values) over categoriesRanges (unresolved ranges)
          const rangeOrValue = quota || categoriesRanges?.[songType]?.[category];
          const { min, max } = calculateRangeFromSettings(rangeOrValue, mode, total);

          if (max > 0) {
            const baseMatcher = (song) => {
              const group = getSongTypeGroup(song.songType);
              let cat = (song.songCategory || '').toLowerCase();
              // Normalize "no category" (with space) to "noCategory" (camelCase)
              if (cat === 'no category') {
                cat = 'noCategory';
              }
              const normalizedCategory = category.toLowerCase() === 'no category' ? 'noCategory' : category.toLowerCase();
              return group === songType && cat === normalizedCategory;
            };
  ...
  'songs-and-types': {
    // Global filtering: eliminate songs of types that are completely disabled
    applyGlobalFilter: (songs, settings) => {
      const { types, typesRanges } = settings;

      // Determine which song types are enabled (have any quota > 0)
      const enabledTypes = {
        openings: false,
        endings: false,
        inserts: false
      ...
      ['openings', 'endings', 'inserts'].forEach(typeKey => {
        // Check if type has a static value or range with max > 0
        const rangeOrValue = (types?.[typeKey] !== undefined ? types[typeKey] : null) || typesRanges?.[typeKey];
        if (rangeOrValue) {
          // If it's a number > 0, it's enabled
          if (typeof rangeOrValue === 'number' && rangeOrValue > 0) {
            enabledTypes[typeKey] = true;
          }
          // If it's an object with max > 0, it's enabled
          else if (typeof rangeOrValue === 'object' && rangeOrValue.max > 0) {
            enabledTypes[typeKey] = true;
          }
        }
      });

      // If all types are enabled, no filtering needed
      if (enabledTypes.openings && enabledTypes.endings && enabledTypes.inserts) {
        return { songs, stats: null };
      }
      ...
      const filtered = songs.filter(song => {
        ...
        if (songType.startsWith('Opening')) return enabledTypes.openings;
        if (songType.startsWith('Ending')) return enabledTypes.endings;
        if (songType.includes('Insert')) return enabledTypes.inserts;
        return false; // Unknown type - filter out
      });

      console.log(`[GLOBAL FILTERS] Songs & Types filter: ${filtered.length}/${beforeCount} songs (enabled: ${Object.entries(enabledTypes).filter(([k, v]) => v).map(([k]) => k).join(', ')})`);

      const stats = recordFilterStat('Songs & Types', beforeCount, filtered.length, {
        enabledTypes
      });
    ...
    buildBaskets: (settings, targetSourceId, context) => {
      ...
      ['openings', 'endings', 'inserts'].forEach(typeKey => {
        ...
        if (max > 0) {
          ...
          const unresolvedTypeRange = typesRanges?.[typeKey];
          if (unresolvedTypeRange && typeof unresolvedTypeRange === 'object' && 'min' in unresolvedTypeRange && 'max' in unresolvedTypeRange) {
            basket.relaxRange = calculateRangeFromSettings(unresolvedTypeRange, mode, total);
          }
          baskets.push(basket);
        }
      });

      // Build song selection baskets (random vs watched vs unwatched)
      // - random: Songs from anywhere (can be on list or not)
      // - watched: Songs from player's list
      // - unwatched: Songs NOT on player's list (from masterlist)
      const hasSelectionConfig = songSelection && (
        songSelection.random > 0 ||
        songSelection.watched > 0 ||
        songSelection.unwatched > 0
      );

      if (hasSelectionConfig) {
        ...
        let adjustedUnwatched = songSelection.unwatched || 0;
        ...
        // Create random basket (matches ANY source type - both watched and unwatched)
        if (adjustedRandom > 0) {
          const randomRange = (songSelection.random !== undefined && songSelection.random !== null)
            ? { min: songSelection.random, max: songSelection.random }
            : songSelectionRanges?.random;
          const { min, max } = calculateRangeFromSettings(randomRange, mode, total);

          if (max > 0) {
            // Random matches any source (watched, unwatched, or untagged)
            const baseMatcher = (song) => true; // Accept any song
            const basket = createBasket(
            ...
            if (songSelectionRanges?.random) {
              basket.relaxRange = calculateRangeFromSettings(songSelectionRanges.random, mode, total);
            }
            baskets.push(basket);
          }
        }

        // Create watched basket (from player's list only)
        if (adjustedWatched > 0) {
          let watchedRange = (songSelection.watched !== undefined && songSelection.watched !== null)
            ? { min: songSelection.watched || 0, max: songSelection.watched || 0 }
            : songSelectionRanges?.watched;
          if (hasMasterlistNodes && songSelection.random > 0) {
            const randomRange = (songSelection.random !== undefined && songSelection.random !== null)
              ? { min: songSelection.random || 0, max: songSelection.random || 0 }
              : songSelectionRanges?.random;
            watchedRange = {
          ...
          if (max > 0) {
            ...
            if (songSelectionRanges?.watched) {
              basket.relaxRange = calculateRangeFromSettings(songSelectionRanges.watched, mode, total);
            }
            baskets.push(basket);
          }
        }

        // Create unwatched basket (from masterlist, NOT on player's list)
        if (adjustedUnwatched > 0) {
          const unwatchedRange = (songSelection.unwatched !== undefined && songSelection.unwatched !== null)
            ? { min: songSelection.unwatched, max: songSelection.unwatched }
            : songSelectionRanges?.unwatched;
          const { min, max } = calculateRangeFromSettings(unwatchedRange, mode, total);

          if (max > 0) {
            const baseMatcher = (song) => song._sourceType === 'unwatched';
            const basket = createBasket(
              `songSelection-unwatched-${sourceSuffix}`,
              min,
              max,
              wrapMatcherWithSourceCheck(baseMatcher, targetSourceIds)
            );
            if (songSelectionRanges?.unwatched) {
              basket.relaxRange = calculateRangeFromSettings(songSelectionRanges.unwatched, mode, total);
            }
            baskets.push(basket);
          }
        }
    ...
    buildBaskets: (settings, targetSourceId, context) => {
      ...
      if (useManyListsMode) {
        ...
        if (viewMode === 'basic' && difficulties) {
          const basicRanges = [
            { label: 'easy', diffMin: 60, diffMax: 100, quota: difficulties.easy, enabled: difficulties.easy?.enabled !== false, isExclusiveMin: true },
            { label: 'medium', diffMin: 25, diffMax: 60, quota: difficulties.medium, enabled: difficulties.medium?.enabled !== false, isExclusiveMin: true },
            { label: 'hard', diffMin: 0, diffMax: 25, quota: difficulties.hard, enabled: difficulties.hard?.enabled !== false, isExclusiveMin: false }
          ];
          ...
          basicRanges.forEach(range => {
            ...
            if (max > 0) {
              const baseMatcher = (song) => {
                ...
                const matchesMin = range.isExclusiveMin ? diff > range.diffMin : diff >= range.diffMin;
                return matchesMin && diff <= range.diffMax;
              };
          ...
          ranges.forEach((range) => {
            // Prioritize songCount (resolved value) over songCountRange (unresolved range)
            const rangeOrValue = (range.songCount !== undefined ? range.songCount : null) || range.songCountRange;
            if (!rangeOrValue) return;
            ...
            if (max > 0) {
              const baseMatcher = (song) => {
                const diff = song.songDifficulty;
                if (diff === null || diff === undefined) return false;
                // Heuristic: if from is not 0, make it exclusive to avoid double-counting at boundaries
                // which causes shortfall when total of max counts equals total pool size
                const matchesMin = range.from > 0 ? diff > range.from : diff >= range.from;
                return matchesMin && diff <= range.to;
              };
        ...
        if (viewMode === 'basic' && difficulties) {
          const basicRanges = [
            { label: 'easy', diffMin: 60, diffMax: 100, quota: difficulties.easy || 0, isExclusiveMin: true },
            { label: 'medium', diffMin: 25, diffMax: 60, quota: difficulties.medium || 0, isExclusiveMin: true },
            { label: 'hard', diffMin: 0, diffMax: 25, quota: difficulties.hard || 0, isExclusiveMin: false }
          ];
  ...
  'vintage': {
    ...
    buildBaskets: (settings, targetSourceId) => {
      ...
      if (ranges && ranges.length > 0) {
        ranges.forEach((range) => {
          // Prioritize value (resolved value) over valueRange (unresolved range)
          const rangeOrValue = (range.value !== undefined ? range.value : null) || range.valueRange;
          if (!rangeOrValue) return;
...
async function fetchUserListWithCache(platform, username, selectedLists, fetchFn, forceRefresh = false) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to load user list for ${username} (${platform}): ${errorMessage}`);
    }

    return await response.json();
  } catch (error) {
    // If it's already our error, rethrow it
    if (error.message.includes('Failed to load user list')) {
      throw error;
    }
    // Otherwise, wrap it with context
    throw new Error(`Failed to load user list for ${username} (${platform}): ${error.message}`);
  }
}
...
async function loadSingleSongList(songListSettings, fetchFn = fetch) {
  if (songListSettings.mode === 'masterlist') {
    ...
    try {
      ...
      console.error(`[SONG LOADING] Error loading user list for ${username} (${platform}):`, error);
      console.error(`[SONG LOADING] Selected lists:`, JSON.stringify(selectedLists));
      console.error(`[SONG LOADING] Error stack:`, error.stack);
      throw new Error(`Failed to load user list for ${username} (${platform}): ${error.message}`);
    }
  }
...
async function batchFetchUserLists(userListSources, fetchFn = fetch) {
  ...
  const fetchPromises = Array.from(userRequestMap.entries()).map(async ([key, requestData]) => {
    try {
      ...
      console.error(`[BATCH FETCH] Error loading user list for ${requestData.username} (${requestData.platform}):`, error);
      console.error(`[BATCH FETCH] Request data:`, JSON.stringify({
        username: requestData.username,
        platform: requestData.platform,
        selectedLists: requestData.selectedLists,
        sourceCount: requestData.sources.length
      }));
      console.error(`[BATCH FETCH] Error stack:`, error.stack);
...
export async function loadSourceSongs(simulatedConfig, fetchFn = fetch) {
  ...
  // Check if songSelection (random/watched/unwatched) is configured
  ...
  const hasUnwatched = hasSongSelection?.unwatched > 0;

  if (hasSongSelection && (hasRandom || hasWatched || hasUnwatched)) {
    console.log(`[SONG LOADING] Song selection detected: Random=${hasSongSelection.random || 0}, Watched=${hasSongSelection.watched || 0}, Unwatched=${hasSongSelection.unwatched || 0}`);
    ...
    // If random or unwatched is enabled, load masterlist if there's no masterlist source node already
    // If a masterlist source node exists, it's already tagged as 'watched' above
    // Note: Both random and unwatched need masterlist songs (songs not on player list)
    if ((hasRandom || hasUnwatched) && masterlistNodeIds.size === 0) {
      console.log('[SONG LOADING] Loading masterlist for random/unwatched song selection...');
      ...
      // Tag masterlist songs as 'unwatched' (NOT on player's list) and add as a separate source
      // Note: masterlist is already filtered to exclude songs on user lists
      const taggedMasterlistSongs = uniqueMasterlistSongs.map(song => ({
        ...song,
        _sourceId: 'unwatched-masterlist',
        _sourceInfo: 'Unwatched',
        _sourceType: 'unwatched'
      }));

      songsBySource.push({
        songs: taggedMasterlistSongs,
        percentage: null, // Will be handled by songSelection baskets
        nodeId: 'unwatched-masterlist',
        listInfo: 'Unwatched'
      });
    } else if ((hasRandom || hasUnwatched) && masterlistNodeIds.size > 0) {
      console.log(`[SONG LOADING] Skipping random/unwatched masterlist loading - ${masterlistNodeIds.size} masterlist source node(s) already exist and will be treated as 'watched'`);
    }
  }
...
function applyGlobalFilters(songs, filters, supportsPlayerScore) {
  ...
  for (const filter of filters) {
    ...
    // Capture pre-popularity pool reference before applying popularity filter.
    // This is later used for row-level popularity percentages in final output.
    if (definitionId === 'popularity' && !popularityReference) {
      let maxPopularity = null;
      const sortedPopularities = [];
      for (const song of songsToFilter) {
        if (song.sourceAnime?.popularity !== null && song.sourceAnime?.popularity !== undefined) {
          const popularity = Number(song.sourceAnime.popularity);
          if (Number.isFinite(popularity)) {
            maxPopularity = maxPopularity === null ? popularity : Math.max(maxPopularity, popularity);
            sortedPopularities.push(popularity);
          }
        }
      }

      sortedPopularities.sort((a, b) => a - b);

      popularityReference = {
        poolCount: songsToFilter.length,
        maxPopularity,
        sortedPopularities
      };
    }
...
function buildSongListBaskets(simulatedConfig, rng, songsBySource) {
  ...
  const hasSongSelection = songsAndTypesFilter?.settings?.songSelection;
  const hasSongSelectionConfigured = hasSongSelection && (
    hasSongSelection.random > 0 ||
    hasSongSelection.watched > 0 ||
    hasSongSelection.unwatched > 0
  );

  // Count songs that must come from masterlist (unwatched/random) vs user list (watched)
  const unwatchedCount = hasSongSelection?.unwatched || 0;
  const randomCount = hasSongSelection?.random || 0;
  const masterlistSongsNeeded = unwatchedCount + randomCount;
  ...
  if (shouldSkipSongListBaskets) {
    ...
    console.log(`[BASKETS] SongSelection is configured BUT source percentages exist, creating song list baskets (masterlist songs needed: ${masterlistSongsNeeded})`);
  }
  ...
  if (songsBySource && Array.isArray(songsBySource)) {
    ...
    if (sourceBaskets.length > 0) {
      ...
      for (const basketData of sourceBaskets) {
        let min = basketData.roundedValue;
        let max = basketData.roundedValue;

        // If unwatched/random is configured, reduce the minimum by the masterlist songs needed
        // because those songs will come from the masterlist, not the user's list
        if (masterlistSongsNeeded > 0) {
          const sourceShare = basketData.originalPercentage / 100;
          const adjustedMin = Math.max(0, min - Math.ceil(masterlistSongsNeeded * sourceShare));
          console.log(`[BASKETS] Adjusting song list basket min for ${basketData.source.listInfo}: ${min} → ${adjustedMin} (masterlist songs needed: ${masterlistSongsNeeded})`);
          min = adjustedMin;
        }

        // Create basket even if max is 0, to allow for relaxation later if needed
        ...
        const matcherAllowsMasterlist = masterlistSongsNeeded > 0;
        ...
        baskets.push(createBasket(
          ...
          (song) => {
            ...
            const sourceId = song._basketSourceId || song._sourceId;
            const matchesSource = sourceId === basketData.source.nodeId;

            // When unwatched/random is configured, also accept masterlist songs
            if (matcherAllowsMasterlist && song._sourceType === 'unwatched') {
              return true;
            }
...
function buildBaskets(simulatedConfig, rng, songsBySource = null, songSelectionMode = 'default') {
  ...
  const unwatchedCount = hasSongSelection?.unwatched || 0;
  const randomCount = hasSongSelection?.random || 0;
  const watchedCount = hasSongSelection?.watched || 0;
  const masterlistSongsNeeded = unwatchedCount + randomCount; // Songs that can come from masterlist
 ...
 * @param {Song} song - Song object
 * @returns {string} Anime ID string
 */
function getSongAnimeId(song) {
  return String(song.malId || song.linked_ids?.myanimelist || song.animeENName);
}

/**
 * Build a stable selection identity key
 * @param {Song} song - Song object
 * @returns {string} Stable key for selection/dedup analysis
 */
function getSelectionIdentityKey(song) {
  if (song?.annSongId !== undefined && song?.annSongId !== null && song?.annSongId !== '') {
    return `ann:${String(song.annSongId)}`;
  }

  const fallback = getSongIdentityKey(song);
  if (fallback) {
    return fallback;
  }

  return `fallback:${String(song?.animeENName || '')}|${String(song?.songName || '')}|${String(song?.songType || '')}`;
}

/**
 * Compute the feasible target song count based on hard constraints
 * @param {Song[]} eligibleSongs - Songs that passed global filtering
 * @param {number} requestedTarget - Requested song count from config
 * @param {boolean} allowDuplicateShows - Whether duplicate anime is allowed
 * @returns {{effectiveTarget: number, feasibleCap: number, uniqueAnimeCount: number|null}} Feasibility summary
 */
function getFeasibleTargetCount(eligibleSongs, requestedTarget, allowDuplicateShows) {
  const eligibleCount = Array.isArray(eligibleSongs) ? eligibleSongs.length : 0;
  const cappedRequested = Math.max(0, Math.floor(Number.isFinite(requestedTarget) ? requestedTarget : 0));

  if (allowDuplicateShows) {
    const feasibleCap = eligibleCount;
    return {
      effectiveTarget: Math.min(cappedRequested, feasibleCap),
      feasibleCap,
      uniqueAnimeCount: null
    };
  }

  const uniqueAnimeIds = new Set();
  for (const song of eligibleSongs || []) {
    uniqueAnimeIds.add(getSongAnimeId(song));
  }

  const uniqueAnimeCount = uniqueAnimeIds.size;
  const feasibleCap = Math.min(eligibleCount, uniqueAnimeCount);
  return {
    effectiveTarget: Math.min(cappedRequested, feasibleCap),
    feasibleCap,
    uniqueAnimeCount
...
/**
 * Count songs that satisfy active basket constraints and derive feasible cap
 * @param {Song[]} eligibleSongs - Songs after global filtering
 * @param {Basket[]} baskets - Built baskets (strict settings)
 * @param {boolean} allowDuplicateShows - Whether duplicate anime is allowed
 * @returns {{constraintFeasibleSongCount: number, constraintUniqueAnimeCount: number|null, constraintFeasibleCap: number}} Constraint feasibility metrics
 */
function calculateConstraintFeasibility(eligibleSongs, baskets, allowDuplicateShows) {
  const safeEligibleSongs = Array.isArray(eligibleSongs) ? eligibleSongs : [];
  const safeBaskets = Array.isArray(baskets) ? baskets : [];
  const activeBaskets = safeBaskets.filter((basket) => basket.max > 0);

  const activeTypes = new Set(activeBaskets.map((basket) => getBasketType(basket.id)).filter((type) => type !== 'other'));
  const constraintFeasibleSongs = safeEligibleSongs.filter((song) => {
    if (activeBaskets.length === 0) {
      return true;
    }

    const applicableActiveBaskets = activeBaskets.filter((basket) => basket.matcher(song));
    if (applicableActiveBaskets.length === 0) {
      return false;
    }

    const applicableTypes = new Set(
      applicableActiveBaskets
        .map((basket) => getBasketType(basket.id))
  ...
  // Selection deduplicates by identity, so report feasibility in that same space
  const uniqueConstraintSongs = new Map();
  constraintFeasibleSongs.forEach((song) => {
    uniqueConstraintSongs.set(getSelectionIdentityKey(song), song);
  });
  const uniqueConstraintList = Array.from(uniqueConstraintSongs.values());
  const constraintFeasibleSongCount = uniqueConstraintList.length;
  if (allowDuplicateShows) {
    return {
      constraintFeasibleSongCount,
      constraintUniqueAnimeCount: null,
      constraintFeasibleCap: constraintFeasibleSongCount
    };
  }

  const uniqueAnimeIds = new Set(uniqueConstraintList.map((song) => getSongAnimeId(song)));
  const constraintUniqueAnimeCount = uniqueAnimeIds.size;
  return {
    constraintFeasibleSongCount,
    constraintUniqueAnimeCount,
    constraintFeasibleCap: Math.min(constraintFeasibleSongCount, constraintUniqueAnimeCount)
  };
}

/**
 * Build exclusive basket signature for swap-based repairs
 * @param {Song} song - Song to analyze
 * @param {Basket[]} baskets - All baskets
 * @param {Set<string>} activeExclusiveTypes - Active exclusive types
 * @param {WeakMap<object, object>} signatureCache - Cache for signatures
 * @returns {Object|null} Signature object or null if not exclusive
 */
function getExclusiveSignature(song, baskets, activeExclusiveTypes, signatureCache) {
  if (signatureCache.has(song)) {
    return signatureCache.get(song);
  }

  const applicableBaskets = getApplicableBaskets(song, baskets);
  const typeToBasket = new Map();

  for (const type of activeExclusiveTypes) {
    const matches = applicableBaskets.filter(b => getBasketType(b.id) === type);
    if (matches.length !== 1) {
      signatureCache.set(song, null);
      return null;
    }
    typeToBasket.set(type, matches[0].id);
  }

  const songListKey = applicableBaskets
    .filter(b => b.id.startsWith('songList-'))
    .map(b => b.id)
    .sort()
    .join('|');
  ...
  signatureCache.set(song, signature);
  return signature;
}

/**
 * Check if swapping songs would keep all baskets within bounds
 * @param {Map<string, Basket>} basketMap - Map of basket ID to basket
 * @param {Basket[]} removeBaskets - Applicable baskets for removed song
 * @param {Basket[]} addBaskets - Applicable baskets for added song
 * @returns {boolean} True if swap is safe
 */
function canSwapWithoutBreakingBaskets(basketMap, removeBaskets, addBaskets) {
  const delta = new Map();

  removeBaskets.forEach(b => {
    delta.set(b.id, (delta.get(b.id) || 0) - 1);
  });
  addBaskets.forEach(b => {
    delta.set(b.id, (delta.get(b.id) || 0) + 1);
  });

  for (const [id, change] of delta.entries()) {
    if (change === 0) continue;
    const basket = basketMap.get(id);
    if (!basket) continue;
    const newCount = basket.current + change;
    if (newCount < basket.min || newCount > basket.max) {
      return false;
    }
  }
...
/**
 * Attempt to repair basket deadlocks via targeted swaps and additions
 * @param {Object} context - Song addition context
 * @param {Basket[]} baskets - All baskets
 * @param {Song[]} eligibleSongs - All eligible songs
 * @param {boolean} allowDuplicateShows - Duplicate show setting
 * @param {() => number} rng - RNG function
 * @param {Function} tryAddSong - Song addition function
 * @param {Object} [options] - Repair options
 * @returns {{swapsMade:number, addsMade:number}} Repair results
 */
function repairDistributionDeadlocks(
  context,
  baskets,
  eligibleSongs,
  allowDuplicateShows,
  ...
  const { selectedSongs, selectedIds, animeCount, selectedAnime } = context;
  const basketMap = new Map(baskets.map(b => [b.id, b]));
  const exclusiveTypes = new Set(['songType', 'difficulty', 'vintage', 'animeType', 'songSelection']);
  const activeExclusiveTypes = new Set(
    baskets
      .map(b => getBasketType(b.id))
      .filter(type => exclusiveTypes.has(type))
  );

  if (activeExclusiveTypes.size === 0) {
    return { swapsMade: 0, addsMade: 0 };
  }
  ...
  const buildOtherKey = (signature, targetType) => {
    const parts = [signature.songListKey];
    for (const type of activeExclusiveTypes) {
      if (type === targetType) continue;
      parts.push(signature.typeToBasket.get(type));
    }
    return parts.join('|');
  };

  while (progress && iterations < maxIterations && (swapsMade < maxSwaps || addsMade < maxAdds)) {
    iterations++;
    progress = false;

    const basketsNeedingMin = baskets
      .filter(b => b.current < b.min)
      .sort((a, b) => (b.min - b.current) - (a.min - a.current));

    if (basketsNeedingMin.length === 0 && selectedSongs.length >= context.maxTotal) {
      break;
    }

    // Attempt direct adds to satisfy missing baskets
    if (addsMade < maxAdds && selectedSongs.length < context.maxTotal) {
      const unselectedSongs = eligibleSongs.filter(song => !selectedIds.has(song.annSongId));
      const shuffled = shuffleArray([...unselectedSongs], rng);

      for (const target of basketsNeedingMin) {
        let added = false;
        for (const candidate of shuffled) {
          const applicableBaskets = getApplicableBaskets(candidate, baskets);
          if (!applicableBaskets.some(b => b.id === target.id)) continue;
          if (tryAddSong(candidate, 2, null, false)) {
            addsMade++;
            progress = true;
            added = true;
            break;
    ...
    // Attempt targeted swaps for exclusive baskets
    for (const target of basketsNeedingMin) {
      if (swapsMade >= maxSwaps) break;
      const targetType = getBasketType(target.id);
      if (!activeExclusiveTypes.has(targetType)) continue;

      const selectedIndex = new Map();
      for (let i = 0; i < selectedSongs.length; i++) {
        const song = selectedSongs[i];
        const signature = getExclusiveSignature(song, baskets, activeExclusiveTypes, signatureCache);
        if (!signature) continue;
        const otherKey = buildOtherKey(signature, targetType);
        if (!selectedIndex.has(otherKey)) {
          selectedIndex.set(otherKey, []);
        }
        selectedIndex.get(otherKey).push({ song, signature, index: i });
      }
      ...
      let swapDone = false;
      for (const candidate of shuffled) {
        const candidateSignature = getExclusiveSignature(candidate, baskets, activeExclusiveTypes, signatureCache);
        if (!candidateSignature) continue;
        if (candidateSignature.typeToBasket.get(targetType) !== target.id) continue;

        const otherKey = buildOtherKey(candidateSignature, targetType);
        const possibleRemovals = selectedIndex.get(otherKey) || [];

        for (const removal of possibleRemovals) {
          if (removal.signature.typeToBasket.get(targetType) === target.id) continue;

          const candidateAnimeId = getSongAnimeId(candidate);
          const removalAnimeId = getSongAnimeId(removal.song);

          if (!allowDuplicateShows) {
            if (selectedAnime.has(candidateAnimeId) && candidateAnimeId !== removalAnimeId) {
              continue;
            }
          }

          const removeBaskets = removal.signature.applicableBaskets;
          const addBaskets = candidateSignature.applicableBaskets;

          if (!canSwapWithoutBreakingBaskets(basketMap, removeBaskets, addBaskets)) {
            continue;
          }

          // Apply swap
          selectedSongs[removal.index] = candidate;
          selectedIds.delete(removal.song.annSongId);
          selectedIds.add(candidate.annSongId);

          removeBaskets.forEach(b => { b.current--; });
          addBaskets.forEach(b => { b.current++; });

          if (!allowDuplicateShows) {
            if (candidateAnimeId !== removalAnimeId) {
              selectedAnime.delete(removalAnimeId);
              selectedAnime.add(candidateAnimeId);
            }
          } else {
            const removeCount = animeCount.get(removalAnimeId) || 0;
            if (removeCount <= 1) {
              animeCount.delete(removalAnimeId);
            } else {
              animeCount.set(removalAnimeId, removeCount - 1);
            }
            animeCount.set(candidateAnimeId, (animeCount.get(candidateAnimeId) || 0) + 1);
          }
        ...
        if (swapDone || swapsMade >= maxSwaps) break;
      }
...
function compareScores(scoreA, scoreB) {
  // Primary: more songs selected (count stability/fill is top priority)
  ...
  // Tertiary: higher user overlap score
...
/**
 * Build immutable basket baseline constraints
 * @param {Basket[]} baskets - Baskets to snapshot
 * @returns {Map<string, {min: number, max: number, relaxRange?: {min:number,max:number}|null}>} Baseline constraints
 */
function buildBasketBaseline(baskets) {
  const baseline = new Map();
  baskets.forEach((basket) => {
    baseline.set(basket.id, {
      min: basket.min,
      max: basket.max,
      relaxRange: basket.relaxRange || null
    });
  });
  return baseline;
}
...
/**
 * Apply a deterministic relaxation level to basket constraints
 * @param {Basket[]} baskets - Baskets to mutate
 * @param {Map<string, {min: number, max: number, relaxRange?: {min:number,max:number}|null}>} baseline - Immutable baseline constraints
 * @param {Map<string, number>} basketEligibleCounts - Eligible count by basket ID
 * @param {number} targetTotal - Effective target for distribution
 * @param {number} level - Relaxation level (0=strict,1=mins relaxed,2=mins relaxed+max broadened)
 * @returns {void}
 */
function applyRelaxationLevel(baskets, baseline, basketEligibleCounts, targetTotal, level) {
  baskets.forEach((basket) => {
    const base = baseline.get(basket.id) || { min: 0, max: 0, relaxRange: null };
    const eligibleForBasket = basketEligibleCounts.get(basket.id) || 0;
    const isFixedExact = base.min === base.max;
    const relaxRange = base.relaxRange || null;
    ...
    // Preserve exact quotas unless they came from a random range.
    if (isFixedExact) {
      if (level >= 2 && relaxRange && relaxRange.max > base.max) {
        basket.min = Math.max(0, Math.min(base.min, relaxRange.min ?? 0));
        basket.max = Math.min(targetTotal, relaxRange.max);
      } else {
        basket.min = base.min;
        basket.max = base.max;
      }
    ...
    if (level === 2 && relaxRange) {
      basket.min = Math.max(0, Math.min(base.min, relaxRange.min ?? 0));
      basket.max = Math.min(targetTotal, Math.max(base.max, relaxRange.max));
      return;
    }

    basket.min = 0;
    basket.max = Math.min(targetTotal, Math.max(base.max, eligibleForBasket));
  });
}
 ...
 * @param {number} maxAttempts - Maximum attempts budget (distributed across relaxation levels)
 * @param {Song[]} sourceSongs - All source songs (before filtering) for accurate user counting
 * @returns {Song[]} Selected songs
 */
function distributeToBaskets(eligibleSongs, baskets, maxTotal, rng, allowDuplicateShows = true, songSelectionMode = 'default', maxAttempts = 2, sourceSongs = null) {
  ...
  let bestResult = null;
  let bestScore = null;
  const relaxationLevels = [0, 1, 2];
  const attemptsPerLevel = Math.max(1, Math.ceil(maxAttempts / relaxationLevels.length));
  const baselineConstraints = buildBasketBaseline(baskets);

  // Pre-compute how many eligible songs can match each basket
  // This helps us know the maximum achievable count for each basket
  const basketEligibleCounts = new Map();
  baskets.forEach(basket => {
    const matchingCount = eligibleSongs.filter(song => basket.matcher(song)).length;
    basketEligibleCounts.set(basket.id, matchingCount);
  });

  let globalAttempt = 0;
  for (const level of relaxationLevels) {
    console.log(`\n${colors.brightYellow}[DISTRIBUTION] Relaxation level ${level} (${level === 0 ? 'strict' : level === 1 ? 'min relaxed' : 'max broadened'})${colors.reset}`);

    applyRelaxationLevel(baskets, baselineConstraints, basketEligibleCounts, maxTotal, level);
    baskets.forEach((basket) => {
      const eligibleForBasket = basketEligibleCounts.get(basket.id) || 0;
      console.log(`  ${colors.dim}${basket.id}: [${basket.min}-${basket.max}] (${eligibleForBasket} eligible)${colors.reset}`);
    });

    for (let levelAttempt = 0; levelAttempt < attemptsPerLevel && globalAttempt < maxAttempts; levelAttempt++) {
      globalAttempt++;
      console.log(`\n${colors.blue}[DISTRIBUTION] Attempt ${globalAttempt}/${maxAttempts} (level ${level}, local ${levelAttempt + 1}/${attemptsPerLevel})${colors.reset}`);

      const attemptRng = makeRng(`attempt-${level}-${levelAttempt}-${rng()}`);
      ...
      console.log(`${colors.dim}[DISTRIBUTION] Attempt ${globalAttempt} score: ${score.basketsMeetingMin}/${score.totalBaskets} baskets, ${score.songsSelected} songs, user overlap: ${score.userOverlapScore}, proximity: ${score.proximityScore.toFixed(2)}${colors.reset}`);
      ...
      // Stop early when we reached the effective target
      if (score.songsSelected >= maxTotal) {
        console.log(`${colors.bgGreen}${colors.black}[DISTRIBUTION] SUCCESS! Reached target song count on attempt ${globalAttempt}.${colors.reset}`);
        break;
      }
    }

    if (bestScore && bestScore.songsSelected >= maxTotal) {
      break;
    }
  ...
  if (!bestResult || !bestScore) {
    console.log(`\n${colors.brightRed}[DISTRIBUTION] No valid distribution result found.${colors.reset}`);
    return [];
  }
...
function createSongAdder(context, baskets, allowDuplicateShows, rng) {
  ...
  return (song, phase, tierUserCount = null, trackRejections = false) => {
    ...
    if (!allowDuplicateShows) {
      ...
      // Duplicate shows are allowed; do not probabilistically reroll.
    }
...
function processTierPhase1(tierGroup, tryAddSong, baskets, context, maxTotal, maxUserCount, orderedGroups, allowDuplicateShows, rng) {
  ...
  if (selectedSongs.length > 0 && tierGroup.count === maxUserCount) {
    ...
    if (swapsMade > 0) {
      ...
      console.log(`${colors.yellow}[SWAP] No valid swaps found. Failures: DifferentBaskets=${failures.differentBaskets}, DuplicateAnime=${failures.duplicateAnime}, NoBasketSpace=${failures.noBasketSpace}, AlreadySelected=${failures.alreadySelected}${colors.reset}`);
    }
  }
...
function processTierPhase2(tierGroup, tryAddSong, baskets, context, maxTotal, maxUserCount, rng) {
  ...
  while (tierProgress && selectedSongs.length < maxTotal && tierIterations < maxTierIterations) {
    ...
    if (tierGroup.count === maxUserCount) {
      console.log(`${colors.dim}  [TIER ITER ${tierIterations}] Added ${iterationAdded} songs. Rejections: AlreadySelected=${iterationRejections.alreadySelected}, DuplicateAnime=${iterationRejections.duplicateAnime}, NoBasketSpace=${iterationRejections.noBasketSpace}, MaxReached=${iterationRejections.maxReached}${colors.reset}`);
    }
  ...
  // Log tier summary
  if (tierSongsAdded > 0 || (rejectionStats.alreadySelected + rejectionStats.duplicateAnime + rejectionStats.noBasketSpace) > 0) {
    ...
    const totalRejections = rejectionStats.duplicateAnime + rejectionStats.noBasketSpace;
...
function distributeSingleAttempt(eligibleSongs, baskets, maxTotal, rng, allowDuplicateShows, songSelectionMode, attemptNumber, sourceSongs = null) {
    ...
    if (context.selectedSongs.length > 0 && orderedGroups.length > 0) {
      ...
      if (maxTierGroup.count === maxUserCount) {
        ...
        if (swapsMade > 0) {
          ...
          console.log(`${colors.yellow}[SWAP P2] No valid swaps found. Failures: DifferentBaskets=${failures.differentBaskets}, DuplicateAnime=${failures.duplicateAnime}, NoBasketSpace=${failures.noBasketSpace}, AlreadySelected=${failures.alreadySelected}${colors.reset}`);
        }
      }
    }
  } else {
  ...
  // PHASE 2 REPAIR: Attempt targeted swaps/adds to satisfy basket minimums
  const basketsNeedingMinAfterP2 = baskets.filter(b => b.current < b.min);
  if (basketsNeedingMinAfterP2.length > 0 || context.selectedSongs.length < maxTotal) {
    console.log(`${colors.brightYellow}[REPAIR] Attempting targeted repair for ${basketsNeedingMinAfterP2.length} failing basket(s)...${colors.reset}`);
    const { swapsMade, addsMade } = repairDistributionDeadlocks(
      context,
      baskets,
      eligibleSongs,
      allowDuplicateShows,
    ...
    if (swapsMade > 0 || addsMade > 0) {
      console.log(`${colors.brightGreen}[REPAIR] Completed with ${swapsMade} swap(s) and ${addsMade} add(s)${colors.reset}`);
    } else {
      console.log(`${colors.yellow}[REPAIR] No valid repair actions found${colors.reset}`);
    }
  }

  // PHASE 3 COMPLETION: deterministically maximize fill
  if (context.selectedSongs.length < maxTotal) {
    const beforeCompletion = context.selectedSongs.length;
    const remainingSongs = shuffleArray(
      eligibleSongs.filter((song) => !context.selectedIds.has(song.annSongId)),
      rng
    );
    ...
    const completionAdded = context.selectedSongs.length - beforeCompletion;
    if (completionAdded > 0) {
      console.log(`${colors.brightGreen}[COMPLETION] Added ${completionAdded} songs (now ${context.selectedSongs.length}/${maxTotal})${colors.reset}`);
    }
  }
...
function collectSongStatistics(songs) {
  const stats = {
    ...
    playerScoreCount: {},
    animeScoreCount: {},
    popularityStats: { min: Infinity, max: -Infinity, total: 0, count: 0 },
    genreCount: {},
    tagCount: {},
  ...
  songs.forEach((song) => {
    ...
    // Popularity
    // @ts-ignore - popularity exists on sourceAnime at runtime (from AniList enrichment)
    if (song.sourceAnime?.popularity !== null && song.sourceAnime?.popularity !== undefined) {
      // @ts-ignore
      const popularity = Number(song.sourceAnime.popularity);
      if (Number.isFinite(popularity)) {
        stats.popularityStats.min = Math.min(stats.popularityStats.min, popularity);
        stats.popularityStats.max = Math.max(stats.popularityStats.max, popularity);
        stats.popularityStats.total += popularity;
        stats.popularityStats.count++;
      }
    }
...
function displaySongStatistics(stats, songs, songsBySource) {
  ...
  if (stats.popularityStats.count > 0) {
    const avgPopularity = (stats.popularityStats.total / stats.popularityStats.count).toFixed(0);
    console.log(`  ${colors.brightGreen}Anime Popularity:${colors.reset} min=${colors.brightRed}${stats.popularityStats.min.toLocaleString()}${colors.reset}, avg=${colors.brightYellow}${avgPopularity}${colors.reset}, max=${colors.brightGreen}${stats.popularityStats.max.toLocaleString()}${colors.reset} ${colors.dim}(${stats.popularityStats.count} songs)${colors.reset}`);
  }
 ...
 * @param {Array} songsBySource - Array of song sources with their songs
 * @param {{poolCount: number, maxPopularity: number|null, sortedPopularities: number[]}|null} popularityReference - Pre-popularity pool reference
 * @returns {void}
 */
function displayFinalSongList(songs, targetCount, songsBySource = null, popularityReference = null) {
  ...
  console.log(`${colors.bright}${colors.cyan}${'='.repeat(120)}${colors.reset}`);
  ...
  const hasReference = popularityReference && Number.isFinite(popularityReference.poolCount);
  const maxPopularityInPool = hasReference ? popularityReference.maxPopularity : null;
  const popularityPoolCount = hasReference ? popularityReference.poolCount : songs.length;
  const sortedPopularities =
    hasReference && Array.isArray(popularityReference.sortedPopularities)
      ? popularityReference.sortedPopularities
      : [];

  console.log(
    `${colors.dim}[INFO] P: raw popularity + % relative to pre-popularity pool (${popularityPoolCount} songs).` +
    `${maxPopularityInPool !== null ? ` pool max=${maxPopularityInPool.toLocaleString()}` : ''}${colors.reset}`
  );
  ...
  songs.forEach((song, index) => {
    ...
    // Calculate popularity for display: raw value + percentile rank in pre-popularity pool.
    let popularityDisplay = '?';
    if (song.sourceAnime?.popularity !== null && song.sourceAnime?.popularity !== undefined) {
      const popularity = Number(song.sourceAnime.popularity);
      if (Number.isFinite(popularity)) {
        let popularityPercent = 100;

        if (sortedPopularities.length > 0) {
          // Upper-bound binary search gives count of values <= popularity.
          let lo = 0;
          let hi = sortedPopularities.length;
          while (lo < hi) {
            const mid = (lo + hi) >> 1;
            if (sortedPopularities[mid] <= popularity) {
              lo = mid + 1;
            } else {
              hi = mid;
            }
          }
          popularityPercent = Math.round((lo / sortedPopularities.length) * 100);
        } else if (maxPopularityInPool && maxPopularityInPool > 0) {
          // Fallback for older runs where only max popularity is available.
          popularityPercent = Math.round((popularity / maxPopularityInPool) * 100);
        }

        popularityDisplay = `${popularity.toLocaleString()}(${popularityPercent}%)`;
      }
    }
    ...
    console.log(
      ...
      `${colors.brightYellow}P:${popularityDisplay.padEnd(14, ' ')}${colors.reset} ` +
...
/**
 * Filter out songs that have media fields but no media attached.
 * A song is considered "unuploaded" if it declares any of HQ/MQ/audio
 * and all three are falsy (null/empty).
 * @param {Song[]} songs
 * @returns {{songs: Song[], removedCount: number}}
 */
function filterUnuploadedSongs(songs) {
  if (!Array.isArray(songs) || songs.length === 0) {
    return { songs: songs || [], removedCount: 0 };
  }

  const filtered = songs.filter((song) => {
    const hasMediaKeys = Object.prototype.hasOwnProperty.call(song, 'HQ') ||
      Object.prototype.hasOwnProperty.call(song, 'MQ') ||
      Object.prototype.hasOwnProperty.call(song, 'audio');

    if (!hasMediaKeys) {
      return true;
    }

    const hasAnyMedia = Boolean(song.HQ || song.MQ || song.audio);
    return hasAnyMedia;
  });

  return { songs: filtered, removedCount: songs.length - filtered.length };
}
 ...
 */
export async function generateQuizSongs(simulatedConfig, fetchFn = fetch, baseSeed = null, excludedAnnSongIds = []) {
  const { numberOfSongs, filters } = simulatedConfig;
  // Generate 16 random letters if no seed provided
  const seed = baseSeed || generateRandomSeed();
  const rng = makeRng(seed);

  // Normalize excludedAnnSongIds to strings for consistent comparison
  const excludedSet = new Set((excludedAnnSongIds || []).map(id => String(id)));
  if (excludedSet.size > 0) {
    console.log(`[FILTER] Excluding ${excludedSet.size} previously played songs from generation.`);
  }
  ...
  const positiveSongLists = extractPositiveSongListSettings(simulatedConfig);
  if (positiveSongLists.length === 0) {
    throw new Error('No positive song source nodes found in configuration');
  }
  const positiveConfig = {
    ...simulatedConfig,
    songLists: positiveSongLists
  };
  let { songs: sourceSongs, supportsPlayerScore, loadingErrors, songsBySource } = await loadSourceSongs(
    positiveConfig,
    fetchFn
  );

  // PHASE 1.5: Exclude songs from negative song source nodes
  const { songs: negativeSourceSongs, loadingErrors: negativeLoadingErrors } = await loadNegativeSourceSongs(
    simulatedConfig,
    fetchFn
  );
  if (negativeLoadingErrors.length > 0) {
    loadingErrors.push(...negativeLoadingErrors);
  }

  if (negativeSourceSongs.length > 0) {
    const negativeSongKeys = new Set(
      negativeSourceSongs
        .map((song) => getSongIdentityKey(song))
        .filter((key) => key !== null)
    );

    if (negativeSongKeys.size > 0) {
      const beforeCount = sourceSongs.length;
      sourceSongs = sourceSongs.filter((song) => {
        const key = getSongIdentityKey(song);
        return key ? !negativeSongKeys.has(key) : true;
      });
      const removedCount = beforeCount - sourceSongs.length;
      console.log(
        `[FILTER] Removed ${removedCount} songs from negative song sources (${negativeSongKeys.size} exclusion keys).`
      );

      if (songsBySource) {
        songsBySource.forEach((source) => {
          if (source.songs) {
            source.songs = source.songs.filter((song) => {
              const key = getSongIdentityKey(song);
              return key ? !negativeSongKeys.has(key) : true;
            });
          }
  ...
  if (excludedSet.size > 0) {
    const beforeCount = sourceSongs.length;
    // Convert song.annSongId to string for comparison with excludedSet
    sourceSongs = sourceSongs.filter(song => !song.annSongId || !excludedSet.has(String(song.annSongId)));
    const afterCount = sourceSongs.length;
    console.log(`[FILTER] Removed ${beforeCount - afterCount} excluded songs from source pool.`);

    // Also filter songsBySource to keep counts consistent
    if (songsBySource) {
      songsBySource.forEach(source => {
        if (source.songs) {
          source.songs = source.songs.filter(song => !song.annSongId || !excludedSet.has(String(song.annSongId)));
        }
      });
    }
  }
  ...
  // Filter out songs with no media attached (HQ/MQ/audio all falsy)
  const unuploadedResult = filterUnuploadedSongs(sourceSongs);
  if (unuploadedResult.removedCount > 0) {
    console.log(`[FILTER] Removed ${unuploadedResult.removedCount} unuploaded songs (HQ/MQ/audio all falsy).`);
  }
  sourceSongs = unuploadedResult.songs;

  if (songsBySource.length > 0) {
    let removedFromSources = 0;
    songsBySource.forEach(source => {
      if (source.songs) {
        const sourceResult = filterUnuploadedSongs(source.songs);
        source.songs = sourceResult.songs;
        removedFromSources += sourceResult.removedCount;
      }
    });
    if (removedFromSources > 0) {
      console.log(`[FILTER] Removed ${removedFromSources} unuploaded songs from source breakdown.`);
    }
  }
  ...
  // Extract duplicate shows setting early (needed by metadata in all return paths)
  ...
  if (hasUseEntirePool) {
    ...
    popularityReference = filterResult.popularityReference || null;
  }
  ...
  if (eligibleSongs.length === 0) {
    ...
    return {
      ...
      metadata: {
        ...
        eligibleSongCount: 0,
        constraintFeasibleSongCount: 0,
        constraintUniqueAnimeCount: allowDuplicateShows ? null : 0,
        constraintFeasibleCap: 0,
        basketStatus: [],
        failedBaskets: [],
        filterStatistics: filterStatistics,
        loadingErrors: loadingErrors,
  ...
  // Duplicate shows setting (already extracted above for all code paths)
  ...
  console.log(`[SONG GENERATION] Duplicate shows: ${allowDuplicateShows ? 'ENABLED' : 'DISABLED (unique anime only)'}`);
  const feasibleTargetInfo = getFeasibleTargetCount(eligibleSongs, numberOfSongs, allowDuplicateShows);
  const effectiveTarget = feasibleTargetInfo.effectiveTarget;
  if (allowDuplicateShows) {
    console.log(`[SONG GENERATION] Target feasibility: requested=${numberOfSongs}, effective=${effectiveTarget}, eligible cap=${feasibleTargetInfo.feasibleCap}`);
  } else {
    console.log(`[SONG GENERATION] Target feasibility: requested=${numberOfSongs}, effective=${effectiveTarget}, unique anime cap=${feasibleTargetInfo.uniqueAnimeCount}, eligible cap=${feasibleTargetInfo.feasibleCap}`);
  }
  ...
  const constraintFeasibility = calculateConstraintFeasibility(eligibleSongs, baskets, allowDuplicateShows);
  console.log(
    `[SONG GENERATION] Constraint-feasible songs: ${constraintFeasibility.constraintFeasibleSongCount}` +
      (constraintFeasibility.constraintUniqueAnimeCount !== null
        ? `, unique anime: ${constraintFeasibility.constraintUniqueAnimeCount}`
        : '') +
      `, feasible cap: ${constraintFeasibility.constraintFeasibleCap}`
  );

  if (isTrainingMode) {
    console.log('[SONG GENERATION] Training mode detected – filtering songs by basket criteria (respecting 0-max but ignoring other min/max)');
    ...
    const trainingPool = eligibleSongs.filter(song => {
      ...
      // CRITICAL: If a song only matches baskets with max=0, it's not eligible even in training mode
      // This ensures 0% settings in the connector/live-node are respected
      const hasValidBasket = applicableBaskets.some(b => b.max > 0);
      if (!hasValidBasket && applicableBaskets.length > 0) {
        // If it matches some baskets but ALL of them have max=0, it's not eligible
        return false;
      }
      ...
      // AND that basket must have a max > 0 (to respect 0% settings)
      for (const type of basketTypes) {
        if (type !== 'other') {
          const matchingBasketsOfType = applicableBaskets.filter(b => getBasketType(b.id) === type);
          const hasActiveBasketOfType = matchingBasketsOfType.some(b => b.max > 0);

          if (!hasActiveBasketOfType) {
            // There's no basket of this type with max > 0 that the song matches
            return false;
          }
        }
      }
    ...
    selectedSongs = distributeToBaskets(eligibleSongs, baskets, effectiveTarget, rng, allowDuplicateShows, songSelectionMode, 8, sourceSongs);

    console.log(`[SONG GENERATION] Selected ${selectedSongs.length}/${effectiveTarget} songs (requested ${numberOfSongs})`);
  ...
  if (selectedSongs.length > 0 && !isTrainingMode) {
    displayFinalSongList(selectedSongs, effectiveTarget, songsBySource, popularityReference);
  }
  if (isTrainingMode) {
  ...
  // Build song overlap map for duel mode - maps annSongId to all usernames who have this song
  const songToUsersMap = buildSongToUsersMap(songsBySource);
  const songOverlapMapArray = selectedSongs
    .filter(song => song.annSongId)
    .map(song => {
      const users = songToUsersMap.get(song.annSongId) || [];
      return {
        annSongId: song.annSongId,
        hasAnimeUsernames: users.map(u => u.username).filter(Boolean)
      };
    });
  ...
  return {
    ...
    metadata: {
      ...
      constraintFeasibleSongCount: constraintFeasibility.constraintFeasibleSongCount,
      constraintUniqueAnimeCount: constraintFeasibility.constraintUniqueAnimeCount,
      constraintFeasibleCap: constraintFeasibility.constraintFeasibleCap,
      ...
      songOverlapMap: songOverlapMapArray // Array of {annSongId, hasAnimeUsernames} for duel mode - all users who know this song
    }
  };
}
...
// Test-only export for unit testing the distribution algorithm directly
export const __test_distributeToBaskets = distributeToBaskets;
export const __test_filterUnuploadedSongs = filterUnuploadedSongs;
