 ...
 * @param {string|null} targetSourceId - Target source ID to scope to
  ...
  return {
    ...
    buildBaskets: (settings, targetSourceId, context) => {
      ...
      if (context.useManyListsMode) {
        ...
        baskets.push(createBasket(
          `${scoreType}-score-aggregate-${min}-${max}-${targetSourceId || 'all'}`,
          0,
          context.numberOfSongs,
          targetSourceId ? (song) => song._sourceId === targetSourceId && baseMatcher(song) : baseMatcher
        ));
...
const FILTER_REGISTRY = {
  ...
  'anime-score': createScoreRangeFilterProcessor({
    ...
    extractScore: (song) => song.sourceAnime?.averageScore
  }),
      ...
      Object.entries(categories || {}).forEach(([songType, cats]) => {
        Object.entries(cats).forEach(([category, quota]) => {
          ...
          if (max > 0) {
            ...
            baskets.push(createBasket(
              `category-${songType}-${category}-${targetSourceId || 'all'}`,
              min,
              max,
              targetSourceId ? (song) => song._sourceId === targetSourceId && baseMatcher(song) : baseMatcher
            ));
    ...
    buildBaskets: (settings, targetSourceId, context) => {
      ...
      ['openings', 'endings', 'inserts'].forEach(typeKey => {
        ...
        if (max > 0) {
          ...
          baskets.push(createBasket(
            `songType-${typeKey}-${targetSourceId || 'all'}`,
            min,
            max,
            wrapMatcherWithSourceCheck(baseMatcher, targetSourceId)
          ));
      ...
      if (songSelection && (songSelection.random > 0 || songSelection.watched > 0)) {
        ...
        if (adjustedRandom > 0) {
          ...
          if (max > 0) {
            ...
            baskets.push(createBasket(
              `songSelection-random-${targetSourceId || 'all'}`,
              min,
              max,
              wrapMatcherWithSourceCheck(baseMatcher, targetSourceId)
            ));
        ...
        if (adjustedWatched > 0) {
          ...
          if (max > 0) {
            ...
            baskets.push(createBasket(
              `songSelection-watched-${targetSourceId || 'all'}`,
              min,
              max,
              wrapMatcherWithSourceCheck(baseMatcher, targetSourceId)
            ));
        ...
        if (viewMode === 'basic' && difficulties) {
          ...
          basicRanges.forEach(range => {
            ...
            if (max > 0) {
              ...
              baskets.push(createBasket(
                `difficulty-${range.label}-${targetSourceId || 'all'}`,
                min,
                max,
                targetSourceId ? (song) => song._sourceId === targetSourceId && baseMatcher(song) : baseMatcher
              ));
          ...
          ranges.forEach((range) => {
            ...
            if (max > 0) {
              ...
              baskets.push(createBasket(
                `difficulty-${range.from}-${range.to}-${targetSourceId || 'all'}`,
                min,
                max,
                targetSourceId ? (song) => song._sourceId === targetSourceId && baseMatcher(song) : baseMatcher
              ));
      ...
      if (ranges && ranges.length > 0) {
        ranges.forEach((range) => {
          ...
          if (max > 0) {
            ...
            baskets.push(createBasket(
              `vintage-${range.from.season}${range.from.year}-${range.to.season}${range.to.year}-${targetSourceId || 'all'}`,
              min,
              max,
              wrapMatcherWithSourceCheck(baseMatcher, targetSourceId)
            ));
...
function applyGlobalFilters(songs, filters, supportsPlayerScore) {
  ...
  for (const filter of filters) {
    const { definitionId, settings, targetSourceId } = filter;
    console.log(`[GLOBAL FILTERS] Processing filter: ${definitionId}, targetSourceId=${targetSourceId || 'all sources'}`);
    ...
    // If this filter has a targetSourceId, we only apply it to songs from that source
    let songsToFilter = filtered;
    let songsNotToFilter = [];

    if (targetSourceId) {
      songsToFilter = filtered.filter(s => s._sourceId === targetSourceId);
      songsNotToFilter = filtered.filter(s => s._sourceId !== targetSourceId);
      console.log(`[GLOBAL FILTERS] Filter scoped to source ${targetSourceId}: filtering ${songsToFilter.length} songs, keeping ${songsNotToFilter.length} songs untouched`);
    }

    // Apply filter using processor
    const result = processor.applyGlobalFilter(songsToFilter, settings, targetSourceId, context);
...
function buildBaskets(simulatedConfig, rng, songsBySource = null, songSelectionMode = 'default') {
  ...
  for (const filter of filters) {
    const { definitionId, settings, targetSourceId } = filter;
    console.log(`[BASKETS] Processing filter: ${definitionId}, targetSourceId=${targetSourceId || 'all'}`);
    ...
    const filterBaskets = processor.buildBaskets(settings, targetSourceId, context);
