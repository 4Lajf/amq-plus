 ...
 * @param {string|null} targetSourceId - Optional source ID to restrict baskets to
...
export function createScoreBaskets(scoreType, settings, numberOfSongs, targetSourceId = null) {
  ...
  const createMatcher = (scoreValue, isRemaining = false) => {
    ...
    // Wrap with source check if targetSourceId exists
    return targetSourceId
      ? (song) => song._sourceId === targetSourceId && baseMatcher(song)
      : baseMatcher;
  };
  ...
  Object.entries(counts).forEach(([score, count]) => {
    ...
    if (songCount > 0) {
      baskets.push(
        createBasket(
          `${scoreType}Score-${score}-${targetSourceId || 'all'}`,
          songCount,
          songCount,
  ...
  if (totalExplicitCount < numberOfSongs) {
    ...
    baskets.push(
      createBasket(
        `${scoreType}Score-remaining-${targetSourceId || 'all'}`,
        remainingCount,
        remainingCount,
