<script>
	...
	import { importUserList } from '$lib/utils/songListUtils.js';
		...
		try {
			...
			if (profileType === 'mal' && result.songsList.length > 0) {
				...
				if (anilistIdsFromSongs.length > 0) {
					...
					// Create a map of MAL ID -> enriched anime object for quick song enrichment
					const enrichedAnimeMap = new Map();
					...
					result.animeList = result.animeList.map((anime) => {
						...
						if (anilistData) {
							...
							enrichedAnimeMap.set(anime.malId, enrichedAnime);
							return enrichedAnime;
						}
						enrichedAnimeMap.set(anime.malId, anime);
						return anime;
					});

					// CRITICAL: Also enrich the songsList so they have the new metadata/tags
					result.songsList = result.songsList.map((song) => {
						const songMalId = song.malId || song.linked_ids?.myanimelist;
						const enrichedAnime = enrichedAnimeMap.get(songMalId);
						if (enrichedAnime) {
							return {
								...song,
								sourceAnime: enrichedAnime
							};
						}
						return song;
					});
		...
		{#if cacheInfo}
			<div class="text-muted-foreground text-xs">
				...
				{:else if cacheInfo.expiresAt}
					✓ Fresh data cached until {new Date(cacheInfo.expiresAt).toISOString()}
				{/if}
			</div>
		{/if}
