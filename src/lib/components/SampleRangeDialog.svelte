<script>
	 ...
	 * }}
	 */
	let { open = $bindable(false), song = $bindable(null), onSave } = $props();
	...
	function initializeSampleRanges() {
		...
		if (hasCustomSampleRanges) {
			sampleRanges = song.sampleRanges.map((range) => ({
		...
		if (enrichedSong.guessTime !== undefined) {
			if (
				typeof enrichedSong.guessTime === 'object' &&
				enrichedSong.guessTime !== null &&
				enrichedSong.guessTime.useRange
			) {
		...
		if (enrichedSong.extraGuessTime !== undefined) {
			if (
				typeof enrichedSong.extraGuessTime === 'object' &&
				enrichedSong.extraGuessTime !== null &&
				enrichedSong.extraGuessTime.useRange
			) {
	...
	$effect(() => {
		if (song) {
			...
			// Reset state when song is null to prevent undefined bindings
	...
	function handleStartChange(newStart) {
		if (sampleRanges[selectedRangeIndex]) {
			sampleRanges[selectedRangeIndex].start = newStart;
			// Force reactivity
	...
	function handleEndChange(newEnd) {
		if (sampleRanges[selectedRangeIndex]) {
			sampleRanges[selectedRangeIndex].end = newEnd;
			// Force reactivity
			sampleRanges = [...sampleRanges];
									...
									<div class="mb-2 flex items-center justify-between">
										<div class="flex items-center gap-2">
											<Checkbox
												...
												checked={!!useCustomSampleRanges}
												onCheckedChange={(checked) => {
													useCustomSampleRanges = !!checked;
												}}
												class="h-4 w-4"
											/>
											<Label
											...
											<div class="space-y-2">
												...
												>
													<Checkbox
														...
														checked={sampleRanges[selectedRangeIndex]?.randomStartPosition ?? false}
														onCheckedChange={(checked) => {
															if (sampleRanges[selectedRangeIndex]) {
																sampleRanges[selectedRangeIndex].randomStartPosition = checked ?? false;
																// When unchecked, set end to song length
																if (!checked) {
																	const max = getSliderMax();
																	sampleRanges[selectedRangeIndex].end = max;
																}
																// Force reactivity
																sampleRanges = [...sampleRanges];
															}
													...
													<div class="space-y-0.5">
														...
														<Input
															...
															on:change={(e) => {
																...
																if (!Number.isNaN(parsed) && sampleRanges[selectedRangeIndex]) {
																	const normalized = normalizeToStep(parsed);
																	const max = getSliderMax();
																	sampleRanges[selectedRangeIndex].start = normalized;
																	sampleRanges[selectedRangeIndex].end = max; // End is always song length when random start is off
																	// Force reactivity
																	sampleRanges = [...sampleRanges];
									...
									<div class="rounded border bg-gray-50 p-2">
										<div class="mb-1.5 flex items-center justify-between">
											...
											<Checkbox
												...
												checked={!!useCustomGuessTime}
												onCheckedChange={(checked) => {
													useCustomGuessTime = !!checked;
												}}
												class="h-3.5 w-3.5"
											/>
										</div>
												...
												<div class="space-y-1">
													<div class="flex items-center gap-1.5">
														<Checkbox
															...
															checked={!!guessTimeUseRange}
															onCheckedChange={(checked) => {
																guessTimeUseRange = !!checked;
															}}
															class="h-3 w-3"
														/>
														<Label
												...
												<div class="space-y-1">
													<div class="flex items-center gap-1.5">
														<Checkbox
															...
															checked={!!extraGuessTimeUseRange}
															onCheckedChange={(checked) => {
																extraGuessTimeUseRange = !!checked;
															}}
															class="h-3 w-3"
														/>
														<Label
