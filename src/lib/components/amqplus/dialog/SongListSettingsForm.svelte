<script>
	...
	} from '$lib/utils/songListUtils.js';
	import { processProviderData, PROVIDER_INFO } from '$lib/utils/providerUtils.js';
	...
	let showMyLists = $state(true); // Default to My Lists

	// Effect to handle initial showMyLists value and redirect if not logged in
	$effect(() => {
		if (!session && showMyLists) {
			showMyLists = false;
		}
	});
	...
	async function loadLists(page = 1) {
		...
		try {
			...
			const [saved, publicResult] = await Promise.all([
				// Only fetch saved lists if logged in
				session
					? fetchSavedLists().catch((err) => {
							console.error('Error fetching saved lists:', err);
							return [];
						})
					: Promise.resolve([]),
				fetchPublicLists(filters).catch((err) => {
			...
			// If both are empty and it was the first load, we don't want to loop
			if (savedLists.length === 0 && publicLists.length === 0) {
				// We can set a flag or just let the empty state show
			}
		} catch (error) {
			console.error('Unexpected error in loadLists:', error);
	...
	async function selectList(listId) {
		...
		// Saved lists only preserve personal-list metadata when every song comes from source "list".
		editedValue.savedListHasOnlyListSource = false;
		try {
			const result = await loadListSongs(listId);
			const songs = Array.isArray(result?.songs) ? result.songs : [];
			editedValue.savedListHasOnlyListSource = songs.every((song) => song?.source === 'list');
		} catch (error) {
			console.warn('Failed to inspect saved list source composition:', error);
			editedValue.savedListHasOnlyListSource = false;
		}
