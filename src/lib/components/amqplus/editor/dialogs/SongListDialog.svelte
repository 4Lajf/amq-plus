<script>
	/**
	 * Song List Dialog Wrapper
	 * Wraps the SongListSettingsForm in a dialog component with proper state management.
	 *
	 * @component
	 */
	 ...
	 * @property {Object} value - Current song list configuration
	...
	<Dialog.Content class="max-h-[90vh] max-w-3xl overflow-y-auto" portalProps={{ class: '' }}>
		<Dialog.Header class="">
			<Dialog.Title class="flex items-center gap-2">
				...
				<span>{nodeData?.title || 'Song List Configuration'}</span>
			</Dialog.Title>
			<Dialog.Description class="">
				Configure the base song pool for this node. Choose from entire database, user lists, or
				saved lists.
			</Dialog.Description>
		</Dialog.Header>

		<div class="py-4">
			<SongListSettingsForm bind:editedValue bind:isValid bind:validationMessage {onAutoSave} />
		</div>
