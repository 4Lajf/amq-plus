...
export function simulateQuizConfiguration(nodes, edges, configs, providedSeed = null) {
	...
	for (const node of selectedFilterNodes) {
		...
		const sourceSelectorEdge = edges.find(
			(edge) => edge.target === node.id && edge.targetHandle === 'source-selector'
		);
		let targetSourceId = null;
		if (sourceSelectorEdge) {
			const sourceSelectorNode = nodes.find((n) => n.id === sourceSelectorEdge.source);
			if (sourceSelectorNode && sourceSelectorNode.data.currentValue?.targetSourceId) {
				targetSourceId = sourceSelectorNode.data.currentValue.targetSourceId;
			}
		}

		// Create a key that includes both type and source (null for no source selector)
		const groupKey = `${definitionId}|||${targetSourceId || 'all-sources'}`;

		if (!filtersByTypeAndSource.has(groupKey)) {
			filtersByTypeAndSource.set(groupKey, { definitionId, targetSourceId, nodes: [] });
		}
	...
	for (const [groupKey, group] of filtersByTypeAndSource) {
		const { definitionId, targetSourceId, nodes: nodesOfType } = group;
		...
		if (nodesOfType.length === 1) {
			...
			// Add targetSourceId if present
			if (targetSourceId) {
				resolvedFilter.targetSourceId = targetSourceId;
				browserLog(`[SIMULATION] Filter ${node.data.title} scoped to source: ${targetSourceId}`);
			}
			...
			if (resolvedNodes.length > 0) {
				...
				// Add targetSourceId if present (all nodes in this group have the same targetSourceId)
				if (targetSourceId) {
					resolvedFilter.targetSourceId = targetSourceId;
					browserLog(`[SIMULATION] Merged filter scoped to source: ${targetSourceId}`);
				}
