...
export async function POST({ params, request }) {
	try {
		console.log('[TRAINING QUIZ STATS] Request received for quizId:', params.quizId);
		const { token } = await request.json();

		if (!token || typeof token !== 'string' || token.length !== 64) {
			console.log('[TRAINING QUIZ STATS] Invalid token format');
		...
		console.log('[TRAINING QUIZ STATS] Validating token...');
		...
		console.log('[TRAINING QUIZ STATS] Token validated for user:', userId);
		console.log('[TRAINING QUIZ STATS] Looking up quiz by play token:', quizId);
		...
		if (quizError) {
			console.error('[TRAINING QUIZ STATS] Error fetching quiz:', quizError);
		}

		if (quizError || !quizData) {
			console.log('[TRAINING QUIZ STATS] Quiz not found by play token:', quizId);
			return json({
				success: true,
				hasStats: false,
				stats: null
		...
		if (quizData.user_id !== userId) {
			return json({ error: 'You do not have permission to view this quiz\'s training.' }, { status: 403 });
		}

		console.log('[TRAINING QUIZ STATS] Quiz found:', quizData.name, 'ID:', quizData.id);

		// Fetch training progress for this quiz
		console.log('[TRAINING QUIZ STATS] Fetching progress for quiz ID:', quizData.id);
		...
		const playableRecords = (progressRecords || []).filter(isPlayableProgressRecord);

		// Calculate statistics (matching website logic)
		const now = new Date();
		let totalSongs = playableRecords.length;
		let totalAttempts = 0;
		let totalSuccess = 0;
		let dueToday = 0;
		...
		for (const record of playableRecords) {
			...
			if (dueDateTime) {
				const dueDateNormalized = new Date(dueDateTime.getFullYear(), dueDateTime.getMonth(), dueDateTime.getDate());
				const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
				const isDue = dueDateNormalized <= today;
		...
		console.error('[TRAINING QUIZ STATS] ===== CAUGHT ERROR =====');
		console.error('[TRAINING QUIZ STATS] Error:', err);
		console.error('[TRAINING QUIZ STATS] Error message:', err.message);
		console.error('[TRAINING QUIZ STATS] Error stack:', err.stack);
		console.error('[TRAINING QUIZ STATS] ===========================');
