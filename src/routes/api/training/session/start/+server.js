...
export async function POST({ request, url }) {
  ...
  try {
    ...
    const result = trainingScheduler.computeSessionPlaylist(
      progressRecords || [],
      allSongs,
      sessionLength, // Maximum number of songs (FSRS may select fewer if not enough due/new songs available)
      dueSongPercentage // Configurable split percentage
    );
    ...
    if (invalidSongs.length > 0) {
      ...
      invalidSongs.forEach(song => {
        console.error('[TRAINING SESSION]   -', song.song_key || 'unknown');
      });
      return json({ 
