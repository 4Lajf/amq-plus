  ...
  try {
    ...
    dueSongs.forEach((record, index) => {
      try {
        ...
        console.error('[Reset Due Songs] Error processing song:', record.song_key, err);
      }
    });
    ...
    for (const update of updates) {
      const { error: updateError } = await supabaseAdmin
        ...
        .eq('user_id', userId)
        .eq('quiz_id', quizId)
        .eq('song_key', update.song_key);

      if (updateError) {
        console.error('[Reset Due Songs] Error updating song:', update.song_key, updateError);
      }
    }
