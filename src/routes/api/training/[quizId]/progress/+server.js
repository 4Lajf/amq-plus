...
export async function GET({ params, url, locals: { safeGetSession } }) {
  ...
  try {
    ...
    // Fetch progress
    const { data: progress, error } = await fetchAllPages(() =>
      supabaseAdmin
...
export async function POST({ params, request }) {
  ...
  try {
    ...
    if (!quiz || quiz.user_id !== userId) {
      return json({ error: 'You do not have permission to update this quiz\'s training.' }, { status: 403 });
    }
    ...
    for (const update of updates) {
      try {
        const { songKey, rating, timestamp, success, annSongId, songAnnId } = update;

        const songAnnIdNumeric = songAnnId !== undefined && songAnnId !== null
          ? songAnnId
          : (typeof annSongId === 'number' ? annSongId : null);

        if (!songAnnIdNumeric) {
          console.warn('[TRAINING PROGRESS] Missing song_ann_id in batch update:', update);
          failCount++;
          continue;
        }

        if (success === undefined) {
          console.warn('[TRAINING PROGRESS] Missing success in batch update:', update);
          failCount++;
          continue;
        }
        ...
        const { data: existingProgress } = await supabaseAdmin
          ...
          .eq('user_id', userId)
          .eq('quiz_id', quizId)
          .eq('song_ann_id', songAnnIdNumeric)
          .single();
...
export async function DELETE({ params, request, locals: { safeGetSession } }) {
  ...
  if (!quiz || quiz.user_id !== userId) {
    return json({ error: 'You do not have permission to delete this quiz\'s training.' }, { status: 403 });
  }
