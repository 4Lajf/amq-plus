...
export async function POST({ request }) {
  ...
  try {
    ...
    const requestedSongs = Object.keys(localStorageData).length;
    const reconstructionResult = await reconstructSongsFromLocalStorage(localStorageData);
    const reconstructedMap = reconstructionResult.songMap;
    const failedSongs = reconstructionResult.failedSongs || [];
    ...
    console.log('[IMPORT] Fetching user information...');
    ...
    // Create minimal quiz configuration in v2 routes format
    console.log('[IMPORT] Step 4: Creating quiz configuration (v2 format)');
    const minimalQuizConfig = {
      ...
      routes: [{
        ...
        basicSettings: {
          guessTime: {
            type: 'range', label: 'Guess Time', min: 1, max: 60,
            value: { useRange: false, staticValue: 20, min: 15, max: 25 }
          },
          samplePoint: {
            type: 'complex', label: 'Sample Point',
            value: { useRange: true, staticValue: 20, start: 0, end: 100 }
          },
          playbackSpeed: {
            type: 'complex', label: 'Playback Speed',
            value: { mode: 'static', staticValue: 1, randomValues: [1] }
          },
          duplicateShows: { type: 'boolean', label: 'Duplicate Shows', value: false },
          extraGuessTime: {
            type: 'range', label: 'Extra Guess Time', min: 0, max: 15,
        ...
        numberOfSongs: { useRange: false, staticValue: 20, min: 10, max: 30 },
        filters: []
      }],
      metadata: {
        version: '2.0',
    ...
    if (quizError || !quizData) {
      ...
      return json({ error: 'Failed to create quiz: ' + (quizError?.message || 'Unknown error') }, { status: 500 });
    }

    // Import training progress into the new quiz
    console.log('[IMPORT] Step 5: Importing training progress');
    const result = await importFromLocalStorage(supabaseAdmin, localStorageData, userId, quizData.id, reconstructedMap);
    ...
    return json({
      ...
      failedReconstructionCount: failedSongs.length,
      failedReconstructionSample: failedSongs.slice(0, 25),
      ...result
    });
  } catch (error) {
