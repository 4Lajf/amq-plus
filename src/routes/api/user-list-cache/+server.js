...
async function fetchFreshDataForGuest(platform, username, frontendStatuses, forceRefresh = false) {
	try {
		...
		console.error('[API: User List Cache] fetchFreshDataForGuest error for', { platform, username }, ':', error);
		console.error('[API: User List Cache] Error stack:', error.stack);
		return json(
			{
				success: false,
				error: `Guest fetch error for ${username} (${platform}): ${error.message}`,
				details: {
					platform,
					username,
...
async function handleFetchCache({ request, locals }) {
	...
	let platform, username, selectedLists;

	try {
		const body = await request.json();
		platform = body.platform;
		username = body.username;
		selectedLists = body.selectedLists;
		...
		console.error('[API: User List Cache] handleFetchCache error:', error);
		console.error('[API: User List Cache] Error stack:', error.stack);
		console.error('[API: User List Cache] Request context:', {
			platform: platform || 'unknown',
			username: username || 'unknown',
			selectedLists: selectedLists || 'unknown',
			isAuthenticated
		});
		return json(
			{
				success: false,
				error: `Failed to fetch user list cache${username ? ` for ${username}` : ''}${platform ? ` (${platform})` : ''}: ${error.message}`,
				details: {
					platform,
					username,
					errorType: error.constructor.name
