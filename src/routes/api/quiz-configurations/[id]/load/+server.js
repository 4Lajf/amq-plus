...
export async function GET({ params, locals, url }) {
	...
	try {
		...
		if (!quizData.is_public && !hasValidShareToken && !userIsAdmin) {
			if (!session || !user) {
				throw error(403, { message: 'This quiz is private. You must be logged in as the owner or have a share token to view it.' });
			}
			if (quizData.user_id !== user.id) {
				throw error(403, { message: 'You do not have permission to load this private quiz.' });
			}
		}
		...
		if (quizData.allow_remixing === false && !isOwner && !hasValidShareToken && !userIsAdmin) {
			throw error(403, {
				message: 'This quiz does not allow viewing its configuration (remixing disabled). You can only play it.'
			});
		}
