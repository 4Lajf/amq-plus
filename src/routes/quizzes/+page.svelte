				...
				{#if selectedQuiz}
					...
					{#if selectedQuiz.quiz_metadata}
						...
						<div class="mb-4">
							<div class="flex flex-wrap items-center gap-2">
								...
								<!-- Vintage -->
								{#if selectedQuiz.quiz_metadata.vintage && selectedQuiz.quiz_metadata.vintage.ranges && selectedQuiz.quiz_metadata.vintage.ranges.length > 0}
									{@const vintage = selectedQuiz.quiz_metadata.vintage}
									{@const mode = vintage.mode || 'percentage'}
									{@const isPercentage = mode === 'percentage'}
									
									{#each vintage.ranges as range}
										<Badge variant="outline" class="text-xs text-purple-600" href="">
					...
					<div class="flex flex-col gap-2">
						...
						{:else}
							<Button
								variant="default"
								href={`/quizzes/create?load=${selectedQuiz.id}`}
								class="w-full"
								disabled={false}
							>
								Open in Editor
