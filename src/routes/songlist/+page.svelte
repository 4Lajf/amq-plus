<script>
	...
	let deletedListIds = $state(new Set());
	...
	async function confirmDeleteList() {
		...
		isDeletingId = listToDelete;
		deleteDialogOpen = false;
		toast.info('Deleting list...');
		...
		try {
			...
			// Track deleted list ID so it doesn't reappear when data updates
			deletedListIds.add(listToDelete);
			deletedListIds = new Set(deletedListIds);

			// Remove from favorites if it was favorited
			if (favoriteIds.has(listToDelete)) {
				favoriteIds.delete(listToDelete);
		...
		<div class="lg:col-span-3">
			<Card>
				...
				{@const paginatedLists = favoriteFilter
					? filteredLists.slice(startIndex, endIndex).filter((list) => !deletedListIds.has(list.id))
					: filteredLists.filter((list) => !deletedListIds.has(list.id))}
					...
					{#if totalFilteredItems > 0}
						<div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
							{#each paginatedLists as list}
								<div
									class="relative cursor-pointer rounded-lg border p-4 transition-colors hover:bg-gray-50"
									role="button"
									tabindex="0"
													...
													<DropdownMenu.Content class="" portalProps={{}}>
														<DropdownMenu.Item
															...
															onclick={(e) => {
																e.preventDefault();
																e.stopPropagation();
																if (isDeletingId === list.id) return;
																openDeleteDialog(list.id);
															}}
															disabled={isDeletingId === list.id}
														>
															{#if isDeletingId === list.id}
