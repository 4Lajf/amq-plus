...
<script>
	...
	async function performSave(existingListId) {
		try {
			...
			const creatorUsername = user.user_metadata?.custom_claims.global_name;
	...
	onMount(async () => {
		...
		// If we have both URL id and server-loaded publicList, use server data (faster, handles public lists)
		if (listIdFromUrl && data.publicList && data.publicList.id === listIdFromUrl) {
			// Server already loaded it, use that data
			try {
				if (data.publicList.songs && data.publicList.songs.length > 0) {
					currentSongList = data.publicList.songs.map((s) => {
				...
				toast.error(`Error loading public list: ${error.message}`);
			}
		...
		if (listIdFromUrl && !data.publicList) {
			...
			return; // Don't load publicList if we loaded from URL
		}

		// Fallback: load publicList from server data (for fromList parameter or server-side loads)
		if (data.publicList) {
			// Load the public list for editing - songs are already fetched by server
			try {
				if (data.publicList.songs && data.publicList.songs.length > 0) {
					currentSongList = data.publicList.songs.map((s) => {
						let processedSong = { ...s, source: 'global' };
