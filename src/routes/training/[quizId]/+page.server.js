...
import { calculateQuizStatsWithPool } from '$lib/server/training/training-utils.js';
import masterlistData from '$lib/server/masterlist.json';
...
export async function load({ params, url, locals: { safeGetSession } }) {
  ...
  try {
    ...
    const { data: quiz, error: quizError } = await supabaseAdmin
      ...
      .select('id, user_id, name, description, configuration_data, created_at')
      .eq('id', quizId)
      .single();

    if (quizError || !quiz) {
      console.error('[Training Detail] Quiz not found by ID:', quizId, quizError);
    ...
    const sourceQuizIds = Array.from(new Set((allProgress || []).map(p => p.quiz_id)))
      .filter(id => id && id !== quiz.id);
    ...
    const progressSongIds = new Set(
      (progress || [])
        .map((p) => (p.song_ann_id !== null && p.song_ann_id !== undefined ? String(p.song_ann_id) : ''))
        .filter(Boolean)
    );

    if (progressSongIds.size > 0 && Array.isArray(masterlistData)) {
      for (const song of masterlistData) {
        const annSongId = song.annSongId !== null && song.annSongId !== undefined ? String(song.annSongId) : '';
        if (annSongId && progressSongIds.has(annSongId)) {
    ...
    // When that happens the UI falls back to "Song #123" and "Missing from database".
    ...
    for (const id of progressSongIds) {
      if (!songMetadata[id]) {
        ...
        songMetadata[id] = {
          title: `Song #${id}`,
          artist: 'Missing from database',
          anime: 'Missing from database'
        };
      }
    }
