...
import { generateQuizSongs, parseVintage, isInVintageRange, makeRng, __test_distributeToBaskets, __test_filterUnuploadedSongs } from '../src/lib/server/songFiltering.js';
import { validateSongMatchesFilters, validateBasketDistribution, colorLog, printTestResults } from './utils/testHelpers.js';

// Inline createBasket to avoid Vite import analysis issues
function createBasket(id, min, max, matcher) {
...
describe('Song Filtering - Utility Functions', () => {
  ...
  it('should filter out unuploaded songs with no media', () => {
    const songs = [
      { annSongId: '1', HQ: null, MQ: null, audio: null },
      { annSongId: '2', HQ: null, MQ: 'x.webm', audio: null },
      { annSongId: '3' }
    ];

    const result = __test_filterUnuploadedSongs(songs);
    expect(result.removedCount).toBe(1);
    expect(result.songs.map(s => s.annSongId)).toEqual(['2', '3']);
  });
});
...
describe('Song Filtering - Edge Cases', () => {
  ...
  it('21 - Repair distribution deadlocks (exclusive swaps)', () => {
    const eligibleSongs = [
      { annSongId: 'H_END_1', songName: 'H End 1', songType: 'Ending 1', songDifficulty: 10, animeENName: 'A1' },
      { annSongId: 'E_END_1', songName: 'E End 1', songType: 'Ending 1', songDifficulty: 80, animeENName: 'A2' },
      { annSongId: 'H_END_2', songName: 'H End 2', songType: 'Ending 2', songDifficulty: 5, animeENName: 'A3' },
      { annSongId: 'E_END_2', songName: 'E End 2', songType: 'Ending 2', songDifficulty: 70, animeENName: 'A4' },
      { annSongId: 'H_OP_1', songName: 'H Op 1', songType: 'Opening 1', songDifficulty: 12, animeENName: 'A5' },
      { annSongId: 'E_OP_1', songName: 'E Op 1', songType: 'Opening 1', songDifficulty: 65, animeENName: 'A6' },
      { annSongId: 'H_OP_2', songName: 'H Op 2', songType: 'Opening 2', songDifficulty: 20, animeENName: 'A7' },
      { annSongId: 'E_OP_2', songName: 'E Op 2', songType: 'Opening 2', songDifficulty: 90, animeENName: 'A8' }
    ];

    const baskets = [
      createBasket('songType-openings-all', 2, 4, song => (song.songType || '').startsWith('Opening')),
      createBasket('songType-endings-all', 2, 4, song => (song.songType || '').startsWith('Ending')),
      createBasket('difficulty-0-25-all', 2, 4, song => song.songDifficulty >= 0 && song.songDifficulty < 25),
      createBasket('difficulty-60-100-all', 2, 4, song => song.songDifficulty >= 60 && song.songDifficulty <= 100)
    ];

    const rng = () => 0.999999;
    const selected = __test_distributeToBaskets(eligibleSongs, baskets, 4, rng, true, 'default', 1, null);

    expect(selected.length).toBe(4);

    const openings = selected.filter(s => s.songType?.startsWith('Opening')).length;
    const endings = selected.filter(s => s.songType?.startsWith('Ending')).length;
    const hard = selected.filter(s => s.songDifficulty >= 0 && s.songDifficulty < 25).length;
    const easy = selected.filter(s => s.songDifficulty >= 60 && s.songDifficulty <= 100).length;

    expect(openings).toBeGreaterThanOrEqual(2);
    expect(endings).toBeGreaterThanOrEqual(2);
