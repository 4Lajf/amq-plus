...
function validateTrainingStart() {
  ...
  if (lobby.soloMode) {
    ...
    cslMessage(
      "§CSL0" +
      btoa(
        `${showSelection}§${currentSong}§${totalSongs}§${guessTime}§${extraGuessTime}§${fastSkip ? "1" : "0"
        }`
      )
    );
  }
...
function prepareSongForTraining(songKeys, maxSongs) {
  ...
  finalSelection.forEach((song, index) => {
    console.log(
      `${index + 1}. "${song.songName}" by ${song.songArtist} (Anime: ${song.animeRomajiName
      })`
    );
  });
  ...
  new Listener("New Spectator", (payload) => {
    if (quiz.cslActive && quiz.inQuiz && quiz.isHost) {
      ...
      setTimeout(() => {
        ...
        let message = `${currentSong}§${getStartPoint()}§${song.audio || ""}§${song.video480 || ""
          }§${song.video720 || ""}`;
...
function startQuiz() {
  ...
  setTimeout(() => {
    if (quiz.soloMode) {
      ...
      if (quiz.isHost) {
        let message = `1§${getStartPoint()}§${song.audio || ""}§${song.video480 || ""
          }§${song.video720 || ""}`;
  ...
  setTimeout(() => {
    if (songNumber < totalSongs) {
      if (quiz.soloMode) {
        ...
        if (quiz.isHost) {
          let nextSong = finalSongList[songOrder[songNumber + 1]];
          let message = `${songNumber + 1}§${getStartPoint()}§${nextSong.audio || ""
        ...
        () => {
          ...
          if (quiz.soloMode) {
            ...
            for (let id of Object.keys(correct)) {
              ...
              list.push(
                `${id},${correct[numId] ? "1" : "0"},${pose[numId]},${score[numId]
                }`
              );
            }
            splitIntoChunks(
...
function createSongListTable(displayList = []) {
  ...
  $("#cslgMergeCurrentCount").text(
    `Current song list: ${displayList.length} song${displayList.length === 1 ? "" : "s"
    }`
  );
  $("#cslgSongListWarning").text("");
...
function createMergedSongListTable() {
  ...
  $("#cslgMergeTotalCount").text(
    `Merged song list: ${mergedSongList.length} song${mergedSongList.length === 1 ? "" : "s"
    }`
  );
